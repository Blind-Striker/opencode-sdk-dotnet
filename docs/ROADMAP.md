...
- **Packages** — the two packages publish as `OpenCodeAI.Sdk` and `OpenCodeAI.Sdk.Extensions`
  ...
  Packages, and `0.9.0-preview.2` is on NuGet.org, owned by `OpenCode.NET` and pushed through the
...
4. **M4 — Launcher and process truth.** Parity with upstream's three connection modes: standalone
   ...
   that ignores the first rung. `EnsureAsync` is in flight: the election loop, version policy, and public door land first, with the persistent-terminal sidecar behind a seam until its own slice.
