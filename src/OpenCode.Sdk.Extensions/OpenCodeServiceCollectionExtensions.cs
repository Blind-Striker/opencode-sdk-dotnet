...
public static class OpenCodeServiceCollectionExtensions
{
    ...
    private static IServiceCollection AddOpenCodeCore(IServiceCollection services)
    {
        ...
        _ = services.AddSingleton(static ConfigClient (provider) => provider.GetRequiredService<OpenCodeClient>().Config);
        ...
        _ = services.AddSingleton(static RpcClient (provider) => provider.GetRequiredService<OpenCodeClient>().Rpc);
