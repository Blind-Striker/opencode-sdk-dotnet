// Generated by OpenCode.Sdk.Tools from spec/openapi.json.
// Do not edit by hand — change tools/curation.json or the emitters, then regenerate.
using OpenCode.Sdk.Internal;
using OpenCode.Sdk.Models;

namespace OpenCode.Sdk;
/// <summary>
/// Defines the wire route of every generated operation.
/// </summary>
public static class OpenCodeRoutes
{
    /// <summary>
    /// Defines the &apos;Agents&apos; routes.
    /// </summary>
    public static class Agents
    {
        /// <summary>
        /// The &apos;GET /api/agent/{agentID}&apos; route template.
        /// </summary>
        public const string GetAgentTemplate = "/api/agent/{agentID}";
        /// <summary>
        /// Builds the &apos;/api/agent/{agentID}&apos; route.
        /// </summary>
        /// <param name = "agentId">The &apos;agentID&apos; route value.</param>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string GetAgent(string agentId, AgentRequest? request = null)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(agentId);
            if (agentId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(agentId));
            }

            var path = "/api/agent/" + RouteValuePolicy.Escape(agentId, nameof(agentId));
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;GET /api/agent&apos; route template.
        /// </summary>
        public const string ListAgentsTemplate = "/api/agent";
        /// <summary>
        /// Builds the &apos;/api/agent&apos; route.
        /// </summary>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string ListAgents(AgentListRequest? request = null)
        {
            var path = "/api/agent";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }
    }

    /// <summary>
    /// Defines the &apos;Commands&apos; routes.
    /// </summary>
    public static class Commands
    {
        /// <summary>
        /// The &apos;GET /api/command&apos; route template.
        /// </summary>
        public const string ListCommandsTemplate = "/api/command";
        /// <summary>
        /// Builds the &apos;/api/command&apos; route.
        /// </summary>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string ListCommands(CommandListRequest? request = null)
        {
            var path = "/api/command";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }
    }

    /// <summary>
    /// Defines the &apos;Config&apos; routes.
    /// </summary>
    public static class Config
    {
        /// <summary>
        /// The &apos;GET /api/config/shell&apos; route.
        /// </summary>
        public const string GetShells = "/api/config/shell";
    }

    /// <summary>
    /// Defines the &apos;Credentials&apos; routes.
    /// </summary>
    public static class Credentials
    {
        /// <summary>
        /// The &apos;POST /api/credential/{credentialID}/activate&apos; route template.
        /// </summary>
        public const string ActivateCredentialTemplate = "/api/credential/{credentialID}/activate";
        /// <summary>
        /// Builds the &apos;/api/credential/{credentialID}/activate&apos; route.
        /// </summary>
        /// <param name = "credentialId">The &apos;credentialID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string ActivateCredential(string credentialId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(credentialId);
            if (credentialId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(credentialId));
            }

            return "/api/credential/" + RouteValuePolicy.Escape(credentialId, nameof(credentialId)) + "/activate";
        }

        /// <summary>
        /// The &apos;DELETE /api/credential/{credentialID}&apos; route template.
        /// </summary>
        public const string RemoveCredentialTemplate = "/api/credential/{credentialID}";
        /// <summary>
        /// Builds the &apos;/api/credential/{credentialID}&apos; route.
        /// </summary>
        /// <param name = "credentialId">The &apos;credentialID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string RemoveCredential(string credentialId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(credentialId);
            if (credentialId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(credentialId));
            }

            return "/api/credential/" + RouteValuePolicy.Escape(credentialId, nameof(credentialId));
        }

        /// <summary>
        /// The &apos;PATCH /api/credential/{credentialID}&apos; route template.
        /// </summary>
        public const string UpdateCredentialTemplate = "/api/credential/{credentialID}";
        /// <summary>
        /// Builds the &apos;/api/credential/{credentialID}&apos; route.
        /// </summary>
        /// <param name = "credentialId">The &apos;credentialID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string UpdateCredential(string credentialId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(credentialId);
            if (credentialId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(credentialId));
            }

            return "/api/credential/" + RouteValuePolicy.Escape(credentialId, nameof(credentialId));
        }
    }

    /// <summary>
    /// Defines the &apos;Debug&apos; routes.
    /// </summary>
    public static class Debug
    {
        /// <summary>
        /// The &apos;DELETE /api/debug/location&apos; route template.
        /// </summary>
        public const string EvictLocationTemplate = "/api/debug/location";
        /// <summary>
        /// Builds the &apos;/api/debug/location&apos; route.
        /// </summary>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string EvictLocation(DebugLocationEvictRequest? request = null)
        {
            var path = "/api/debug/location";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;GET /api/debug/location&apos; route.
        /// </summary>
        public const string ListLocations = "/api/debug/location";
    }

    /// <summary>
    /// Defines the &apos;Events&apos; routes.
    /// </summary>
    public static class Events
    {
        /// <summary>
        /// The &apos;GET /api/event&apos; route.
        /// </summary>
        public const string Subscribe = "/api/event";
    }

    /// <summary>
    /// Defines the &apos;Experimental&apos; routes.
    /// </summary>
    public static class Experimental
    {
        /// <summary>
        /// The &apos;POST /api/experimental/session/{sessionID}/skill&apos; route template.
        /// </summary>
        public const string ActivateSessionSkillTemplate = "/api/experimental/session/{sessionID}/skill";
        /// <summary>
        /// Builds the &apos;/api/experimental/session/{sessionID}/skill&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string ActivateSessionSkill(string sessionId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            return "/api/experimental/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/skill";
        }

        /// <summary>
        /// The &apos;POST /api/experimental/integration/wellknown&apos; route template.
        /// </summary>
        public const string AddIntegrationWellknownTemplate = "/api/experimental/integration/wellknown";
        /// <summary>
        /// Builds the &apos;/api/experimental/integration/wellknown&apos; route.
        /// </summary>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string AddIntegrationWellknown(ExperimentalIntegrationWellknownAddRequest? request = null)
        {
            var path = "/api/experimental/integration/wellknown";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;PUT /api/experimental/mcp/{server}&apos; route template.
        /// </summary>
        public const string AddMcpServerTemplate = "/api/experimental/mcp/{server}";
        /// <summary>
        /// Builds the &apos;/api/experimental/mcp/{server}&apos; route.
        /// </summary>
        /// <param name = "server">The &apos;server&apos; route value.</param>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string AddMcpServer(string server, ExperimentalMcpAddRequest? request = null)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(server);
            if (server is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(server));
            }

            var path = "/api/experimental/mcp/" + RouteValuePolicy.Escape(server, nameof(server));
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;POST /api/experimental/mcp/{server}/connect&apos; route template.
        /// </summary>
        public const string ConnectMcpServerTemplate = "/api/experimental/mcp/{server}/connect";
        /// <summary>
        /// Builds the &apos;/api/experimental/mcp/{server}/connect&apos; route.
        /// </summary>
        /// <param name = "server">The &apos;server&apos; route value.</param>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string ConnectMcpServer(string server, ExperimentalMcpConnectRequest? request = null)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(server);
            if (server is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(server));
            }

            var path = "/api/experimental/mcp/" + RouteValuePolicy.Escape(server, nameof(server)) + "/connect";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;POST /api/experimental/mcp/{server}/disconnect&apos; route template.
        /// </summary>
        public const string DisconnectMcpServerTemplate = "/api/experimental/mcp/{server}/disconnect";
        /// <summary>
        /// Builds the &apos;/api/experimental/mcp/{server}/disconnect&apos; route.
        /// </summary>
        /// <param name = "server">The &apos;server&apos; route value.</param>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string DisconnectMcpServer(string server, ExperimentalMcpDisconnectRequest? request = null)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(server);
            if (server is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(server));
            }

            var path = "/api/experimental/mcp/" + RouteValuePolicy.Escape(server, nameof(server)) + "/disconnect";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;POST /api/experimental/generate&apos; route.
        /// </summary>
        public const string GenerateText = "/api/experimental/generate";
        /// <summary>
        /// The &apos;GET /api/experimental/session/{sessionID}/export&apos; route template.
        /// </summary>
        public const string GetSessionExportTemplate = "/api/experimental/session/{sessionID}/export";
        /// <summary>
        /// Builds the &apos;/api/experimental/session/{sessionID}/export&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string GetSessionExport(string sessionId, ExperimentalSessionExportRequest? request = null)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            var path = "/api/experimental/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/export";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddBoolean("sanitize", request.Sanitize);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;GET /api/experimental/session/stats&apos; route template.
        /// </summary>
        public const string GetSessionStatsTemplate = "/api/experimental/session/stats";
        /// <summary>
        /// Builds the &apos;/api/experimental/session/stats&apos; route.
        /// </summary>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string GetSessionStats(ExperimentalSessionStatsRequest? request = null)
        {
            var path = "/api/experimental/session/stats";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddText("from", request.From);
            query.AddText("to", request.To);
            query.AddText("project", request.Project);
            query.AddText("timezone", request.Timezone);
            query.AddText("tools", ToWireValue(request.Tools));
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;POST /api/experimental/session/import&apos; route.
        /// </summary>
        public const string ImportSession = "/api/experimental/session/import";
        /// <summary>
        /// The &apos;GET /api/experimental/session/{sessionID}/instructions/entries&apos; route template.
        /// </summary>
        public const string ListSessionInstructionsEntryTemplate = "/api/experimental/session/{sessionID}/instructions/entries";
        /// <summary>
        /// Builds the &apos;/api/experimental/session/{sessionID}/instructions/entries&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string ListSessionInstructionsEntry(string sessionId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            return "/api/experimental/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/instructions/entries";
        }

        /// <summary>
        /// The &apos;DELETE /api/experimental/mcp/{server}&apos; route template.
        /// </summary>
        public const string RemoveMcpServerTemplate = "/api/experimental/mcp/{server}";
        /// <summary>
        /// Builds the &apos;/api/experimental/mcp/{server}&apos; route.
        /// </summary>
        /// <param name = "server">The &apos;server&apos; route value.</param>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string RemoveMcpServer(string server, ExperimentalMcpRemoveRequest? request = null)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(server);
            if (server is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(server));
            }

            var path = "/api/experimental/mcp/" + RouteValuePolicy.Escape(server, nameof(server));
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;DELETE /api/experimental/session/{sessionID}/instructions/entries/{key}&apos; route template.
        /// </summary>
        public const string RemoveSessionInstructionsEntryTemplate = "/api/experimental/session/{sessionID}/instructions/entries/{key}";
        /// <summary>
        /// Builds the &apos;/api/experimental/session/{sessionID}/instructions/entries/{key}&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <param name = "key">The &apos;key&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string RemoveSessionInstructionsEntry(string sessionId, string key)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            ArgumentException.ThrowIfNullOrWhiteSpace(key);
            if (key is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(key));
            }

            return "/api/experimental/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/instructions/entries/" + RouteValuePolicy.Escape(key, nameof(key));
        }

        /// <summary>
        /// The &apos;PUT /api/experimental/session/{sessionID}/instructions/entries/{key}&apos; route template.
        /// </summary>
        public const string SetSessionInstructionsEntryTemplate = "/api/experimental/session/{sessionID}/instructions/entries/{key}";
        /// <summary>
        /// Builds the &apos;/api/experimental/session/{sessionID}/instructions/entries/{key}&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <param name = "key">The &apos;key&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string SetSessionInstructionsEntry(string sessionId, string key)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            ArgumentException.ThrowIfNullOrWhiteSpace(key);
            if (key is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(key));
            }

            return "/api/experimental/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/instructions/entries/" + RouteValuePolicy.Escape(key, nameof(key));
        }

        /// <summary>
        /// The &apos;PATCH /api/experimental/config&apos; route.
        /// </summary>
        public const string UpdateConfig = "/api/experimental/config";
        /// <summary>
        /// The &apos;POST /api/experimental/session/{sessionID}/wait&apos; route template.
        /// </summary>
        public const string WaitForSessionTemplate = "/api/experimental/session/{sessionID}/wait";
        /// <summary>
        /// Builds the &apos;/api/experimental/session/{sessionID}/wait&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string WaitForSession(string sessionId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            return "/api/experimental/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/wait";
        }
    }

    /// <summary>
    /// Defines the &apos;FileSystem&apos; routes.
    /// </summary>
    public static class FileSystem
    {
        /// <summary>
        /// The &apos;GET /api/fs/find&apos; route template.
        /// </summary>
        public const string FindEntriesTemplate = "/api/fs/find";
        /// <summary>
        /// Builds the &apos;/api/fs/find&apos; route.
        /// </summary>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string FindEntries(FsFindRequest request)
        {
            ArgumentNullException.ThrowIfNull(request);
            var path = "/api/fs/find";
            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            query.AddText("query", request.Query);
            query.AddText("type", ToWireValue(request.Type));
            query.AddText("limit", request.Limit);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;GET /api/fs/list&apos; route template.
        /// </summary>
        public const string ListEntriesTemplate = "/api/fs/list";
        /// <summary>
        /// Builds the &apos;/api/fs/list&apos; route.
        /// </summary>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string ListEntries(FsListRequest? request = null)
        {
            var path = "/api/fs/list";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            query.AddText("path", request.Path);
            return path + query.Value;
        }
    }

    /// <summary>
    /// Defines the &apos;Forms&apos; routes.
    /// </summary>
    public static class Forms
    {
        /// <summary>
        /// The &apos;GET /api/form&apos; route template.
        /// </summary>
        public const string ListFormsTemplate = "/api/form";
        /// <summary>
        /// Builds the &apos;/api/form&apos; route.
        /// </summary>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string ListForms(FormListRequest? request = null)
        {
            var path = "/api/form";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }
    }

    /// <summary>
    /// Defines the &apos;Integrations&apos; routes.
    /// </summary>
    public static class Integrations
    {
        /// <summary>
        /// The &apos;POST /api/integration/{integrationID}/connect/command&apos; route template.
        /// </summary>
        public const string BeginCommandConnectionTemplate = "/api/integration/{integrationID}/connect/command";
        /// <summary>
        /// Builds the &apos;/api/integration/{integrationID}/connect/command&apos; route.
        /// </summary>
        /// <param name = "integrationId">The &apos;integrationID&apos; route value.</param>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string BeginCommandConnection(string integrationId, IntegrationCommandConnectRequest? request = null)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(integrationId);
            if (integrationId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(integrationId));
            }

            var path = "/api/integration/" + RouteValuePolicy.Escape(integrationId, nameof(integrationId)) + "/connect/command";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;POST /api/integration/{integrationID}/connect/oauth&apos; route template.
        /// </summary>
        public const string BeginOauthConnectionTemplate = "/api/integration/{integrationID}/connect/oauth";
        /// <summary>
        /// Builds the &apos;/api/integration/{integrationID}/connect/oauth&apos; route.
        /// </summary>
        /// <param name = "integrationId">The &apos;integrationID&apos; route value.</param>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string BeginOauthConnection(string integrationId, IntegrationOauthConnectRequest? request = null)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(integrationId);
            if (integrationId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(integrationId));
            }

            var path = "/api/integration/" + RouteValuePolicy.Escape(integrationId, nameof(integrationId)) + "/connect/oauth";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;DELETE /api/integration/{integrationID}/connect/command/{attemptID}&apos; route template.
        /// </summary>
        public const string CancelCommandConnectionTemplate = "/api/integration/{integrationID}/connect/command/{attemptID}";
        /// <summary>
        /// Builds the &apos;/api/integration/{integrationID}/connect/command/{attemptID}&apos; route.
        /// </summary>
        /// <param name = "integrationId">The &apos;integrationID&apos; route value.</param>
        /// <param name = "attemptId">The &apos;attemptID&apos; route value.</param>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string CancelCommandConnection(string integrationId, string attemptId, IntegrationCommandCancelRequest? request = null)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(integrationId);
            if (integrationId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(integrationId));
            }

            ArgumentException.ThrowIfNullOrWhiteSpace(attemptId);
            if (attemptId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(attemptId));
            }

            var path = "/api/integration/" + RouteValuePolicy.Escape(integrationId, nameof(integrationId)) + "/connect/command/" + RouteValuePolicy.Escape(attemptId, nameof(attemptId));
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;DELETE /api/integration/{integrationID}/connect/oauth/{attemptID}&apos; route template.
        /// </summary>
        public const string CancelOauthConnectionTemplate = "/api/integration/{integrationID}/connect/oauth/{attemptID}";
        /// <summary>
        /// Builds the &apos;/api/integration/{integrationID}/connect/oauth/{attemptID}&apos; route.
        /// </summary>
        /// <param name = "integrationId">The &apos;integrationID&apos; route value.</param>
        /// <param name = "attemptId">The &apos;attemptID&apos; route value.</param>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string CancelOauthConnection(string integrationId, string attemptId, IntegrationOauthCancelRequest? request = null)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(integrationId);
            if (integrationId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(integrationId));
            }

            ArgumentException.ThrowIfNullOrWhiteSpace(attemptId);
            if (attemptId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(attemptId));
            }

            var path = "/api/integration/" + RouteValuePolicy.Escape(integrationId, nameof(integrationId)) + "/connect/oauth/" + RouteValuePolicy.Escape(attemptId, nameof(attemptId));
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;POST /api/integration/{integrationID}/connect/oauth/{attemptID}/complete&apos; route template.
        /// </summary>
        public const string CompleteOauthConnectionTemplate = "/api/integration/{integrationID}/connect/oauth/{attemptID}/complete";
        /// <summary>
        /// Builds the &apos;/api/integration/{integrationID}/connect/oauth/{attemptID}/complete&apos; route.
        /// </summary>
        /// <param name = "integrationId">The &apos;integrationID&apos; route value.</param>
        /// <param name = "attemptId">The &apos;attemptID&apos; route value.</param>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string CompleteOauthConnection(string integrationId, string attemptId, IntegrationOauthCompleteRequest? request = null)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(integrationId);
            if (integrationId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(integrationId));
            }

            ArgumentException.ThrowIfNullOrWhiteSpace(attemptId);
            if (attemptId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(attemptId));
            }

            var path = "/api/integration/" + RouteValuePolicy.Escape(integrationId, nameof(integrationId)) + "/connect/oauth/" + RouteValuePolicy.Escape(attemptId, nameof(attemptId)) + "/complete";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;POST /api/integration/{integrationID}/connect/key&apos; route template.
        /// </summary>
        public const string ConnectWithKeyTemplate = "/api/integration/{integrationID}/connect/key";
        /// <summary>
        /// Builds the &apos;/api/integration/{integrationID}/connect/key&apos; route.
        /// </summary>
        /// <param name = "integrationId">The &apos;integrationID&apos; route value.</param>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string ConnectWithKey(string integrationId, IntegrationConnectKeyRequest? request = null)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(integrationId);
            if (integrationId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(integrationId));
            }

            var path = "/api/integration/" + RouteValuePolicy.Escape(integrationId, nameof(integrationId)) + "/connect/key";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;GET /api/integration/{integrationID}&apos; route template.
        /// </summary>
        public const string GetTemplate = "/api/integration/{integrationID}";
        /// <summary>
        /// Builds the &apos;/api/integration/{integrationID}&apos; route.
        /// </summary>
        /// <param name = "integrationId">The &apos;integrationID&apos; route value.</param>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string Get(string integrationId, IntegrationRequest? request = null)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(integrationId);
            if (integrationId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(integrationId));
            }

            var path = "/api/integration/" + RouteValuePolicy.Escape(integrationId, nameof(integrationId));
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;GET /api/integration/{integrationID}/connect/command/{attemptID}&apos; route template.
        /// </summary>
        public const string GetCommandStatusTemplate = "/api/integration/{integrationID}/connect/command/{attemptID}";
        /// <summary>
        /// Builds the &apos;/api/integration/{integrationID}/connect/command/{attemptID}&apos; route.
        /// </summary>
        /// <param name = "integrationId">The &apos;integrationID&apos; route value.</param>
        /// <param name = "attemptId">The &apos;attemptID&apos; route value.</param>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string GetCommandStatus(string integrationId, string attemptId, IntegrationCommandStatusRequest? request = null)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(integrationId);
            if (integrationId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(integrationId));
            }

            ArgumentException.ThrowIfNullOrWhiteSpace(attemptId);
            if (attemptId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(attemptId));
            }

            var path = "/api/integration/" + RouteValuePolicy.Escape(integrationId, nameof(integrationId)) + "/connect/command/" + RouteValuePolicy.Escape(attemptId, nameof(attemptId));
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;GET /api/integration/{integrationID}/connect/oauth/{attemptID}&apos; route template.
        /// </summary>
        public const string GetOauthStatusTemplate = "/api/integration/{integrationID}/connect/oauth/{attemptID}";
        /// <summary>
        /// Builds the &apos;/api/integration/{integrationID}/connect/oauth/{attemptID}&apos; route.
        /// </summary>
        /// <param name = "integrationId">The &apos;integrationID&apos; route value.</param>
        /// <param name = "attemptId">The &apos;attemptID&apos; route value.</param>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string GetOauthStatus(string integrationId, string attemptId, IntegrationOauthStatusRequest? request = null)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(integrationId);
            if (integrationId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(integrationId));
            }

            ArgumentException.ThrowIfNullOrWhiteSpace(attemptId);
            if (attemptId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(attemptId));
            }

            var path = "/api/integration/" + RouteValuePolicy.Escape(integrationId, nameof(integrationId)) + "/connect/oauth/" + RouteValuePolicy.Escape(attemptId, nameof(attemptId));
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;GET /api/integration&apos; route template.
        /// </summary>
        public const string ListIntegrationsTemplate = "/api/integration";
        /// <summary>
        /// Builds the &apos;/api/integration&apos; route.
        /// </summary>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string ListIntegrations(IntegrationListRequest? request = null)
        {
            var path = "/api/integration";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }
    }

    /// <summary>
    /// Defines the &apos;LanguageModels&apos; routes.
    /// </summary>
    public static class LanguageModels
    {
        /// <summary>
        /// The &apos;GET /api/model/default&apos; route template.
        /// </summary>
        public const string GetDefaultTemplate = "/api/model/default";
        /// <summary>
        /// Builds the &apos;/api/model/default&apos; route.
        /// </summary>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string GetDefault(ModelDefaultRequest? request = null)
        {
            var path = "/api/model/default";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;GET /api/model&apos; route template.
        /// </summary>
        public const string ListModelsTemplate = "/api/model";
        /// <summary>
        /// Builds the &apos;/api/model&apos; route.
        /// </summary>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string ListModels(ModelListRequest? request = null)
        {
            var path = "/api/model";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }
    }

    /// <summary>
    /// Defines the &apos;Location&apos; routes.
    /// </summary>
    public static class Location
    {
        /// <summary>
        /// The &apos;GET /api/location&apos; route template.
        /// </summary>
        public const string GetTemplate = "/api/location";
        /// <summary>
        /// Builds the &apos;/api/location&apos; route.
        /// </summary>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string Get(LocationRequest? request = null)
        {
            var path = "/api/location";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;POST /api/location/reload&apos; route.
        /// </summary>
        public const string ReloadLocations = "/api/location/reload";
    }

    /// <summary>
    /// Defines the &apos;McpServers&apos; routes.
    /// </summary>
    public static class McpServers
    {
        /// <summary>
        /// The &apos;GET /api/mcp/resource&apos; route template.
        /// </summary>
        public const string GetResourceCatalogTemplate = "/api/mcp/resource";
        /// <summary>
        /// Builds the &apos;/api/mcp/resource&apos; route.
        /// </summary>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string GetResourceCatalog(McpResourceCatalogRequest? request = null)
        {
            var path = "/api/mcp/resource";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;GET /api/mcp&apos; route template.
        /// </summary>
        public const string ListMcpServersTemplate = "/api/mcp";
        /// <summary>
        /// Builds the &apos;/api/mcp&apos; route.
        /// </summary>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string ListMcpServers(McpListRequest? request = null)
        {
            var path = "/api/mcp";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }
    }

    /// <summary>
    /// Defines the &apos;Permissions&apos; routes.
    /// </summary>
    public static class Permissions
    {
        /// <summary>
        /// The &apos;GET /api/permission/request&apos; route template.
        /// </summary>
        public const string ListRequestsTemplate = "/api/permission/request";
        /// <summary>
        /// Builds the &apos;/api/permission/request&apos; route.
        /// </summary>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string ListRequests(PermissionRequestListRequest? request = null)
        {
            var path = "/api/permission/request";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;GET /api/permission/saved&apos; route template.
        /// </summary>
        public const string ListSavedTemplate = "/api/permission/saved";
        /// <summary>
        /// Builds the &apos;/api/permission/saved&apos; route.
        /// </summary>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string ListSaved(PermissionSavedListRequest? request = null)
        {
            var path = "/api/permission/saved";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddText("projectID", request.ProjectId);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;DELETE /api/permission/saved/{id}&apos; route template.
        /// </summary>
        public const string RemoveSavedTemplate = "/api/permission/saved/{id}";
        /// <summary>
        /// Builds the &apos;/api/permission/saved/{id}&apos; route.
        /// </summary>
        /// <param name = "id">The &apos;id&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string RemoveSaved(string id)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(id);
            if (id is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(id));
            }

            return "/api/permission/saved/" + RouteValuePolicy.Escape(id, nameof(id));
        }
    }

    /// <summary>
    /// Defines the &apos;PersistentPtys&apos; routes.
    /// </summary>
    public static class PersistentPtys
    {
        /// <summary>
        /// The &apos;POST /api/experimental/persistent-pty/{ptyID}/connect-token&apos; route template.
        /// </summary>
        public const string CreateConnectTokenTemplate = "/api/experimental/persistent-pty/{ptyID}/connect-token";
        /// <summary>
        /// Builds the &apos;/api/experimental/persistent-pty/{ptyID}/connect-token&apos; route.
        /// </summary>
        /// <param name = "ptyId">The &apos;ptyID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string CreateConnectToken(string ptyId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(ptyId);
            if (ptyId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(ptyId));
            }

            return "/api/experimental/persistent-pty/" + RouteValuePolicy.Escape(ptyId, nameof(ptyId)) + "/connect-token";
        }

        /// <summary>
        /// The &apos;POST /api/experimental/session/{sessionID}/terminal&apos; route template.
        /// </summary>
        public const string CreatePersistentPtyTemplate = "/api/experimental/session/{sessionID}/terminal";
        /// <summary>
        /// Builds the &apos;/api/experimental/session/{sessionID}/terminal&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string CreatePersistentPty(string sessionId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            return "/api/experimental/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/terminal";
        }

        /// <summary>
        /// The &apos;GET /api/experimental/persistent-pty/{ptyID}&apos; route template.
        /// </summary>
        public const string GetTemplate = "/api/experimental/persistent-pty/{ptyID}";
        /// <summary>
        /// Builds the &apos;/api/experimental/persistent-pty/{ptyID}&apos; route.
        /// </summary>
        /// <param name = "ptyId">The &apos;ptyID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string Get(string ptyId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(ptyId);
            if (ptyId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(ptyId));
            }

            return "/api/experimental/persistent-pty/" + RouteValuePolicy.Escape(ptyId, nameof(ptyId));
        }

        /// <summary>
        /// The &apos;GET /api/experimental/session/{sessionID}/terminal/read&apos; route template.
        /// </summary>
        public const string GetReadTemplate = "/api/experimental/session/{sessionID}/terminal/read";
        /// <summary>
        /// Builds the &apos;/api/experimental/session/{sessionID}/terminal/read&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string GetRead(string sessionId, PersistentPtyReadRequest? request = null)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            var path = "/api/experimental/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/terminal/read";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddText("lines", request.Lines);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;GET /api/experimental/persistent-pty/{ptyID}/snapshot&apos; route template.
        /// </summary>
        public const string GetSnapshotTemplate = "/api/experimental/persistent-pty/{ptyID}/snapshot";
        /// <summary>
        /// Builds the &apos;/api/experimental/persistent-pty/{ptyID}/snapshot&apos; route.
        /// </summary>
        /// <param name = "ptyId">The &apos;ptyID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string GetSnapshot(string ptyId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(ptyId);
            if (ptyId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(ptyId));
            }

            return "/api/experimental/persistent-pty/" + RouteValuePolicy.Escape(ptyId, nameof(ptyId)) + "/snapshot";
        }

        /// <summary>
        /// The &apos;POST /api/experimental/persistent-pty/handoff&apos; route.
        /// </summary>
        public const string Handoff = "/api/experimental/persistent-pty/handoff";
        /// <summary>
        /// The &apos;GET /api/experimental/session/{sessionID}/terminal&apos; route template.
        /// </summary>
        public const string ListPersistentPtysTemplate = "/api/experimental/session/{sessionID}/terminal";
        /// <summary>
        /// Builds the &apos;/api/experimental/session/{sessionID}/terminal&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string ListPersistentPtys(string sessionId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            return "/api/experimental/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/terminal";
        }

        /// <summary>
        /// The &apos;DELETE /api/experimental/persistent-pty/{ptyID}&apos; route template.
        /// </summary>
        public const string RemoveTemplate = "/api/experimental/persistent-pty/{ptyID}";
        /// <summary>
        /// Builds the &apos;/api/experimental/persistent-pty/{ptyID}&apos; route.
        /// </summary>
        /// <param name = "ptyId">The &apos;ptyID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string Remove(string ptyId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(ptyId);
            if (ptyId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(ptyId));
            }

            return "/api/experimental/persistent-pty/" + RouteValuePolicy.Escape(ptyId, nameof(ptyId));
        }

        /// <summary>
        /// The &apos;POST /api/experimental/persistent-pty/shutdown&apos; route.
        /// </summary>
        public const string Shutdown = "/api/experimental/persistent-pty/shutdown";
        /// <summary>
        /// The &apos;PUT /api/experimental/persistent-pty/{ptyID}&apos; route template.
        /// </summary>
        public const string UpdateTemplate = "/api/experimental/persistent-pty/{ptyID}";
        /// <summary>
        /// Builds the &apos;/api/experimental/persistent-pty/{ptyID}&apos; route.
        /// </summary>
        /// <param name = "ptyId">The &apos;ptyID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string Update(string ptyId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(ptyId);
            if (ptyId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(ptyId));
            }

            return "/api/experimental/persistent-pty/" + RouteValuePolicy.Escape(ptyId, nameof(ptyId));
        }
    }

    /// <summary>
    /// Defines the &apos;Plugins&apos; routes.
    /// </summary>
    public static class Plugins
    {
        /// <summary>
        /// The &apos;POST /api/plugin/check&apos; route template.
        /// </summary>
        public const string CheckPluginUpdatesTemplate = "/api/plugin/check";
        /// <summary>
        /// Builds the &apos;/api/plugin/check&apos; route.
        /// </summary>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string CheckPluginUpdates(PluginCheckRequest? request = null)
        {
            var path = "/api/plugin/check";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;GET /api/plugin&apos; route template.
        /// </summary>
        public const string ListPluginsTemplate = "/api/plugin";
        /// <summary>
        /// Builds the &apos;/api/plugin&apos; route.
        /// </summary>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string ListPlugins(PluginListRequest? request = null)
        {
            var path = "/api/plugin";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;POST /api/plugin/update&apos; route template.
        /// </summary>
        public const string UpdatePluginsTemplate = "/api/plugin/update";
        /// <summary>
        /// Builds the &apos;/api/plugin/update&apos; route.
        /// </summary>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string UpdatePlugins(PluginUpdateRequest? request = null)
        {
            var path = "/api/plugin/update";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }
    }

    /// <summary>
    /// Defines the &apos;Projects&apos; routes.
    /// </summary>
    public static class Projects
    {
        /// <summary>
        /// The &apos;GET /api/project&apos; route.
        /// </summary>
        public const string ListProjects = "/api/project";
        /// <summary>
        /// The &apos;PATCH /api/project/{projectID}&apos; route template.
        /// </summary>
        public const string UpdateProjectTemplate = "/api/project/{projectID}";
        /// <summary>
        /// Builds the &apos;/api/project/{projectID}&apos; route.
        /// </summary>
        /// <param name = "projectId">The &apos;projectID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string UpdateProject(string projectId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(projectId);
            if (projectId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(projectId));
            }

            return "/api/project/" + RouteValuePolicy.Escape(projectId, nameof(projectId));
        }
    }

    /// <summary>
    /// Defines the &apos;Providers&apos; routes.
    /// </summary>
    public static class Providers
    {
        /// <summary>
        /// The &apos;GET /api/provider/{providerID}&apos; route template.
        /// </summary>
        public const string GetProviderTemplate = "/api/provider/{providerID}";
        /// <summary>
        /// Builds the &apos;/api/provider/{providerID}&apos; route.
        /// </summary>
        /// <param name = "providerId">The &apos;providerID&apos; route value.</param>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string GetProvider(string providerId, ProviderRequest? request = null)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(providerId);
            if (providerId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(providerId));
            }

            var path = "/api/provider/" + RouteValuePolicy.Escape(providerId, nameof(providerId));
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;GET /api/provider&apos; route template.
        /// </summary>
        public const string ListProvidersTemplate = "/api/provider";
        /// <summary>
        /// Builds the &apos;/api/provider&apos; route.
        /// </summary>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string ListProviders(ProviderListRequest? request = null)
        {
            var path = "/api/provider";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }
    }

    /// <summary>
    /// Defines the &apos;Ptys&apos; routes.
    /// </summary>
    public static class Ptys
    {
        /// <summary>
        /// The &apos;POST /api/pty/{ptyID}/connect-token&apos; route template.
        /// </summary>
        public const string CreateConnectTokenTemplate = "/api/pty/{ptyID}/connect-token";
        /// <summary>
        /// Builds the &apos;/api/pty/{ptyID}/connect-token&apos; route.
        /// </summary>
        /// <param name = "ptyId">The &apos;ptyID&apos; route value.</param>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string CreateConnectToken(string ptyId, PtyConnectTokenRequest? request = null)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(ptyId);
            if (ptyId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(ptyId));
            }

            var path = "/api/pty/" + RouteValuePolicy.Escape(ptyId, nameof(ptyId)) + "/connect-token";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;POST /api/pty&apos; route template.
        /// </summary>
        public const string CreatePtyTemplate = "/api/pty";
        /// <summary>
        /// Builds the &apos;/api/pty&apos; route.
        /// </summary>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string CreatePty(PtyCreateRequest? request = null)
        {
            var path = "/api/pty";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;GET /api/pty/{ptyID}&apos; route template.
        /// </summary>
        public const string GetTemplate = "/api/pty/{ptyID}";
        /// <summary>
        /// Builds the &apos;/api/pty/{ptyID}&apos; route.
        /// </summary>
        /// <param name = "ptyId">The &apos;ptyID&apos; route value.</param>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string Get(string ptyId, PtyRequest? request = null)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(ptyId);
            if (ptyId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(ptyId));
            }

            var path = "/api/pty/" + RouteValuePolicy.Escape(ptyId, nameof(ptyId));
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;GET /api/pty&apos; route template.
        /// </summary>
        public const string ListPtysTemplate = "/api/pty";
        /// <summary>
        /// Builds the &apos;/api/pty&apos; route.
        /// </summary>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string ListPtys(PtyListRequest? request = null)
        {
            var path = "/api/pty";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;DELETE /api/pty/{ptyID}&apos; route template.
        /// </summary>
        public const string RemoveTemplate = "/api/pty/{ptyID}";
        /// <summary>
        /// Builds the &apos;/api/pty/{ptyID}&apos; route.
        /// </summary>
        /// <param name = "ptyId">The &apos;ptyID&apos; route value.</param>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string Remove(string ptyId, PtyRemoveRequest? request = null)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(ptyId);
            if (ptyId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(ptyId));
            }

            var path = "/api/pty/" + RouteValuePolicy.Escape(ptyId, nameof(ptyId));
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;PUT /api/pty/{ptyID}&apos; route template.
        /// </summary>
        public const string UpdateTemplate = "/api/pty/{ptyID}";
        /// <summary>
        /// Builds the &apos;/api/pty/{ptyID}&apos; route.
        /// </summary>
        /// <param name = "ptyId">The &apos;ptyID&apos; route value.</param>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string Update(string ptyId, PtyUpdateRequest? request = null)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(ptyId);
            if (ptyId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(ptyId));
            }

            var path = "/api/pty/" + RouteValuePolicy.Escape(ptyId, nameof(ptyId));
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }
    }

    /// <summary>
    /// Defines the &apos;References&apos; routes.
    /// </summary>
    public static class References
    {
        /// <summary>
        /// The &apos;GET /api/reference&apos; route template.
        /// </summary>
        public const string ListReferencesTemplate = "/api/reference";
        /// <summary>
        /// Builds the &apos;/api/reference&apos; route.
        /// </summary>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string ListReferences(ReferenceListRequest? request = null)
        {
            var path = "/api/reference";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }
    }

    /// <summary>
    /// Defines the &apos;Rpc&apos; routes.
    /// </summary>
    public static class Rpc
    {
        /// <summary>
        /// The &apos;POST /api/rpc/{rpcID}/{method}&apos; route template.
        /// </summary>
        public const string CallTemplate = "/api/rpc/{rpcID}/{method}";
        /// <summary>
        /// Builds the &apos;/api/rpc/{rpcID}/{method}&apos; route.
        /// </summary>
        /// <param name = "rpcId">The &apos;rpcID&apos; route value.</param>
        /// <param name = "method">The &apos;method&apos; route value.</param>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string Call(string rpcId, string method, RpcCallRequest? request = null)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(rpcId);
            if (rpcId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(rpcId));
            }

            ArgumentException.ThrowIfNullOrWhiteSpace(method);
            if (method is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(method));
            }

            var path = "/api/rpc/" + RouteValuePolicy.Escape(rpcId, nameof(rpcId)) + "/" + RouteValuePolicy.Escape(method, nameof(method));
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }
    }

    /// <summary>
    /// Defines the &apos;Server&apos; routes.
    /// </summary>
    public static class Server
    {
        /// <summary>
        /// The &apos;GET /api/info&apos; route.
        /// </summary>
        public const string GetInfo = "/api/info";
    }

    /// <summary>
    /// Defines the &apos;Sessions&apos; routes.
    /// </summary>
    public static class Sessions
    {
        /// <summary>
        /// The &apos;POST /api/session/{sessionID}/synthetic&apos; route template.
        /// </summary>
        public const string AddSyntheticMessageTemplate = "/api/session/{sessionID}/synthetic";
        /// <summary>
        /// Builds the &apos;/api/session/{sessionID}/synthetic&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string AddSyntheticMessage(string sessionId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            return "/api/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/synthetic";
        }

        /// <summary>
        /// The &apos;DELETE /api/session/{sessionID}/form/{formID}&apos; route template.
        /// </summary>
        public const string CancelFormTemplate = "/api/session/{sessionID}/form/{formID}";
        /// <summary>
        /// Builds the &apos;/api/session/{sessionID}/form/{formID}&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <param name = "formId">The &apos;formID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string CancelForm(string sessionId, string formId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            ArgumentException.ThrowIfNullOrWhiteSpace(formId);
            if (formId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(formId));
            }

            return "/api/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/form/" + RouteValuePolicy.Escape(formId, nameof(formId));
        }

        /// <summary>
        /// The &apos;DELETE /api/session/{sessionID}/inbox/{inboxID}&apos; route template.
        /// </summary>
        public const string CancelInboxTemplate = "/api/session/{sessionID}/inbox/{inboxID}";
        /// <summary>
        /// Builds the &apos;/api/session/{sessionID}/inbox/{inboxID}&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <param name = "inboxId">The &apos;inboxID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string CancelInbox(string sessionId, string inboxId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            ArgumentException.ThrowIfNullOrWhiteSpace(inboxId);
            if (inboxId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(inboxId));
            }

            return "/api/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/inbox/" + RouteValuePolicy.Escape(inboxId, nameof(inboxId));
        }

        /// <summary>
        /// The &apos;DELETE /api/session/{sessionID}/revert&apos; route template.
        /// </summary>
        public const string ClearRevertTemplate = "/api/session/{sessionID}/revert";
        /// <summary>
        /// Builds the &apos;/api/session/{sessionID}/revert&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string ClearRevert(string sessionId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            return "/api/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/revert";
        }

        /// <summary>
        /// The &apos;POST /api/session/{sessionID}/revert/commit&apos; route template.
        /// </summary>
        public const string CommitRevertTemplate = "/api/session/{sessionID}/revert/commit";
        /// <summary>
        /// Builds the &apos;/api/session/{sessionID}/revert/commit&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string CommitRevert(string sessionId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            return "/api/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/revert/commit";
        }

        /// <summary>
        /// The &apos;POST /api/session/{sessionID}/compact&apos; route template.
        /// </summary>
        public const string CompactTemplate = "/api/session/{sessionID}/compact";
        /// <summary>
        /// Builds the &apos;/api/session/{sessionID}/compact&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string Compact(string sessionId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            return "/api/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/compact";
        }

        /// <summary>
        /// The &apos;POST /api/session/{sessionID}/form&apos; route template.
        /// </summary>
        public const string CreateFormTemplate = "/api/session/{sessionID}/form";
        /// <summary>
        /// Builds the &apos;/api/session/{sessionID}/form&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string CreateForm(string sessionId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            return "/api/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/form";
        }

        /// <summary>
        /// The &apos;POST /api/session/{sessionID}/permission&apos; route template.
        /// </summary>
        public const string CreatePermissionTemplate = "/api/session/{sessionID}/permission";
        /// <summary>
        /// Builds the &apos;/api/session/{sessionID}/permission&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string CreatePermission(string sessionId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            return "/api/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/permission";
        }

        /// <summary>
        /// The &apos;POST /api/session&apos; route.
        /// </summary>
        public const string CreateSession = "/api/session";
        /// <summary>
        /// The &apos;POST /api/session/{sessionID}/fork&apos; route template.
        /// </summary>
        public const string ForkTemplate = "/api/session/{sessionID}/fork";
        /// <summary>
        /// Builds the &apos;/api/session/{sessionID}/fork&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string Fork(string sessionId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            return "/api/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/fork";
        }

        /// <summary>
        /// The &apos;POST /api/session/{sessionID}/generate&apos; route template.
        /// </summary>
        public const string GenerateTextTemplate = "/api/session/{sessionID}/generate";
        /// <summary>
        /// Builds the &apos;/api/session/{sessionID}/generate&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string GenerateText(string sessionId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            return "/api/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/generate";
        }

        /// <summary>
        /// The &apos;GET /api/session/{sessionID}&apos; route template.
        /// </summary>
        public const string GetTemplate = "/api/session/{sessionID}";
        /// <summary>
        /// Builds the &apos;/api/session/{sessionID}&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string Get(string sessionId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            return "/api/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId));
        }

        /// <summary>
        /// The &apos;GET /api/session/active&apos; route.
        /// </summary>
        public const string GetActive = "/api/session/active";
        /// <summary>
        /// The &apos;GET /api/session/{sessionID}/context&apos; route template.
        /// </summary>
        public const string GetContextTemplate = "/api/session/{sessionID}/context";
        /// <summary>
        /// Builds the &apos;/api/session/{sessionID}/context&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string GetContext(string sessionId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            return "/api/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/context";
        }

        /// <summary>
        /// The &apos;GET /api/session/{sessionID}/diff&apos; route template.
        /// </summary>
        public const string GetDiffTemplate = "/api/session/{sessionID}/diff";
        /// <summary>
        /// Builds the &apos;/api/session/{sessionID}/diff&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string GetDiff(string sessionId, SessionDiffRequest? request = null)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            var path = "/api/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/diff";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddText("from", request.From);
            query.AddText("to", request.To);
            query.AddText("context", request.Context);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;GET /api/session/{sessionID}/form/{formID}&apos; route template.
        /// </summary>
        public const string GetFormTemplate = "/api/session/{sessionID}/form/{formID}";
        /// <summary>
        /// Builds the &apos;/api/session/{sessionID}/form/{formID}&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <param name = "formId">The &apos;formID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string GetForm(string sessionId, string formId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            ArgumentException.ThrowIfNullOrWhiteSpace(formId);
            if (formId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(formId));
            }

            return "/api/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/form/" + RouteValuePolicy.Escape(formId, nameof(formId));
        }

        /// <summary>
        /// The &apos;GET /api/experimental/session/{sessionID}/log&apos; route template.
        /// </summary>
        public const string GetLogTemplate = "/api/experimental/session/{sessionID}/log";
        /// <summary>
        /// Builds the &apos;/api/experimental/session/{sessionID}/log&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string GetLog(string sessionId, SessionLogRequest? request = null)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            var path = "/api/experimental/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/log";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddText("after", request.After);
            query.AddBoolean("follow", request.Follow);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;GET /api/session/{sessionID}/message/{messageID}&apos; route template.
        /// </summary>
        public const string GetMessageTemplate = "/api/session/{sessionID}/message/{messageID}";
        /// <summary>
        /// Builds the &apos;/api/session/{sessionID}/message/{messageID}&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <param name = "messageId">The &apos;messageID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string GetMessage(string sessionId, string messageId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            ArgumentException.ThrowIfNullOrWhiteSpace(messageId);
            if (messageId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(messageId));
            }

            return "/api/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/message/" + RouteValuePolicy.Escape(messageId, nameof(messageId));
        }

        /// <summary>
        /// The &apos;GET /api/session/{sessionID}/permission/{requestID}&apos; route template.
        /// </summary>
        public const string GetPermissionTemplate = "/api/session/{sessionID}/permission/{requestID}";
        /// <summary>
        /// Builds the &apos;/api/session/{sessionID}/permission/{requestID}&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <param name = "requestId">The &apos;requestID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string GetPermission(string sessionId, string requestId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            ArgumentException.ThrowIfNullOrWhiteSpace(requestId);
            if (requestId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(requestId));
            }

            return "/api/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/permission/" + RouteValuePolicy.Escape(requestId, nameof(requestId));
        }

        /// <summary>
        /// The &apos;POST /api/session/{sessionID}/interrupt&apos; route template.
        /// </summary>
        public const string InterruptTemplate = "/api/session/{sessionID}/interrupt";
        /// <summary>
        /// Builds the &apos;/api/session/{sessionID}/interrupt&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string Interrupt(string sessionId, SessionInterruptRequest? request = null)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            var path = "/api/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/interrupt";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddBoolean("resume", request.Resume);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;GET /api/session/{sessionID}/form&apos; route template.
        /// </summary>
        public const string ListFormsTemplate = "/api/session/{sessionID}/form";
        /// <summary>
        /// Builds the &apos;/api/session/{sessionID}/form&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string ListForms(string sessionId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            return "/api/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/form";
        }

        /// <summary>
        /// The &apos;GET /api/session/{sessionID}/inbox&apos; route template.
        /// </summary>
        public const string ListInboxTemplate = "/api/session/{sessionID}/inbox";
        /// <summary>
        /// Builds the &apos;/api/session/{sessionID}/inbox&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string ListInbox(string sessionId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            return "/api/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/inbox";
        }

        /// <summary>
        /// The &apos;GET /api/session/{sessionID}/message&apos; route template.
        /// </summary>
        public const string ListMessagesTemplate = "/api/session/{sessionID}/message";
        /// <summary>
        /// Builds the &apos;/api/session/{sessionID}/message&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string ListMessages(string sessionId, SessionMessageListRequest? request = null)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            var path = "/api/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/message";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddText("limit", request.Limit);
            query.AddOrder("order", request.Order);
            query.AddText("cursor", request.Cursor);
            query.AddText("type", ToWireValue(request.Type));
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;GET /api/session/{sessionID}/permission&apos; route template.
        /// </summary>
        public const string ListRequestsTemplate = "/api/session/{sessionID}/permission";
        /// <summary>
        /// Builds the &apos;/api/session/{sessionID}/permission&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string ListRequests(string sessionId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            return "/api/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/permission";
        }

        /// <summary>
        /// The &apos;GET /api/session&apos; route template.
        /// </summary>
        public const string ListSessionsTemplate = "/api/session";
        /// <summary>
        /// Builds the &apos;/api/session&apos; route.
        /// </summary>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string ListSessions(SessionListRequest? request = null)
        {
            var path = "/api/session";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddText("limit", request.Limit);
            query.AddOrder("order", request.Order);
            query.AddText("search", request.Search);
            query.AddParentFilter("parentID", request.ParentId);
            query.AddText("directory", request.Directory);
            query.AddText("project", request.Project);
            query.AddText("subpath", request.Subpath);
            query.AddText("cursor", request.Cursor);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;POST /api/session/{sessionID}/view&apos; route template.
        /// </summary>
        public const string MarkViewedTemplate = "/api/session/{sessionID}/view";
        /// <summary>
        /// Builds the &apos;/api/session/{sessionID}/view&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string MarkViewed(string sessionId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            return "/api/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/view";
        }

        /// <summary>
        /// The &apos;POST /api/session/{sessionID}/move&apos; route template.
        /// </summary>
        public const string MoveTemplate = "/api/session/{sessionID}/move";
        /// <summary>
        /// Builds the &apos;/api/session/{sessionID}/move&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string Move(string sessionId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            return "/api/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/move";
        }

        /// <summary>
        /// The &apos;POST /api/session/{sessionID}/background&apos; route template.
        /// </summary>
        public const string MoveToolsToBackgroundTemplate = "/api/session/{sessionID}/background";
        /// <summary>
        /// Builds the &apos;/api/session/{sessionID}/background&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string MoveToolsToBackground(string sessionId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            return "/api/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/background";
        }

        /// <summary>
        /// The &apos;POST /api/session/{sessionID}/prompt&apos; route template.
        /// </summary>
        public const string PromptTemplate = "/api/session/{sessionID}/prompt";
        /// <summary>
        /// Builds the &apos;/api/session/{sessionID}/prompt&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string Prompt(string sessionId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            return "/api/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/prompt";
        }

        /// <summary>
        /// The &apos;DELETE /api/session/{sessionID}&apos; route template.
        /// </summary>
        public const string RemoveTemplate = "/api/session/{sessionID}";
        /// <summary>
        /// Builds the &apos;/api/session/{sessionID}&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string Remove(string sessionId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            return "/api/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId));
        }

        /// <summary>
        /// The &apos;POST /api/session/{sessionID}/form/{formID}/reply&apos; route template.
        /// </summary>
        public const string ReplyToFormTemplate = "/api/session/{sessionID}/form/{formID}/reply";
        /// <summary>
        /// Builds the &apos;/api/session/{sessionID}/form/{formID}/reply&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <param name = "formId">The &apos;formID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string ReplyToForm(string sessionId, string formId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            ArgumentException.ThrowIfNullOrWhiteSpace(formId);
            if (formId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(formId));
            }

            return "/api/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/form/" + RouteValuePolicy.Escape(formId, nameof(formId)) + "/reply";
        }

        /// <summary>
        /// The &apos;POST /api/session/{sessionID}/permission/{requestID}/reply&apos; route template.
        /// </summary>
        public const string ReplyToPermissionTemplate = "/api/session/{sessionID}/permission/{requestID}/reply";
        /// <summary>
        /// Builds the &apos;/api/session/{sessionID}/permission/{requestID}/reply&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <param name = "requestId">The &apos;requestID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string ReplyToPermission(string sessionId, string requestId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            ArgumentException.ThrowIfNullOrWhiteSpace(requestId);
            if (requestId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(requestId));
            }

            return "/api/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/permission/" + RouteValuePolicy.Escape(requestId, nameof(requestId)) + "/reply";
        }

        /// <summary>
        /// The &apos;POST /api/session/{sessionID}/command&apos; route template.
        /// </summary>
        public const string RunCommandTemplate = "/api/session/{sessionID}/command";
        /// <summary>
        /// Builds the &apos;/api/session/{sessionID}/command&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string RunCommand(string sessionId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            return "/api/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/command";
        }

        /// <summary>
        /// The &apos;POST /api/session/{sessionID}/shell&apos; route template.
        /// </summary>
        public const string RunShellCommandTemplate = "/api/session/{sessionID}/shell";
        /// <summary>
        /// Builds the &apos;/api/session/{sessionID}/shell&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string RunShellCommand(string sessionId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            return "/api/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/shell";
        }

        /// <summary>
        /// The &apos;PUT /api/session/{sessionID}/environment&apos; route template.
        /// </summary>
        public const string SetEnvironmentTemplate = "/api/session/{sessionID}/environment";
        /// <summary>
        /// Builds the &apos;/api/session/{sessionID}/environment&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string SetEnvironment(string sessionId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            return "/api/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/environment";
        }

        /// <summary>
        /// The &apos;POST /api/session/{sessionID}/revert/stage&apos; route template.
        /// </summary>
        public const string StageRevertTemplate = "/api/session/{sessionID}/revert/stage";
        /// <summary>
        /// Builds the &apos;/api/session/{sessionID}/revert/stage&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string StageRevert(string sessionId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            return "/api/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/revert/stage";
        }

        /// <summary>
        /// The &apos;POST /api/session/{sessionID}/agent&apos; route template.
        /// </summary>
        public const string SwitchAgentTemplate = "/api/session/{sessionID}/agent";
        /// <summary>
        /// Builds the &apos;/api/session/{sessionID}/agent&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string SwitchAgent(string sessionId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            return "/api/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/agent";
        }

        /// <summary>
        /// The &apos;POST /api/session/{sessionID}/model&apos; route template.
        /// </summary>
        public const string SwitchModelTemplate = "/api/session/{sessionID}/model";
        /// <summary>
        /// Builds the &apos;/api/session/{sessionID}/model&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string SwitchModel(string sessionId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            return "/api/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/model";
        }

        /// <summary>
        /// The &apos;PATCH /api/session/{sessionID}&apos; route template.
        /// </summary>
        public const string UpdateTemplate = "/api/session/{sessionID}";
        /// <summary>
        /// Builds the &apos;/api/session/{sessionID}&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string Update(string sessionId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            return "/api/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId));
        }

        /// <summary>
        /// The &apos;PATCH /api/session/{sessionID}/inbox/{inboxID}&apos; route template.
        /// </summary>
        public const string UpdateInboxTemplate = "/api/session/{sessionID}/inbox/{inboxID}";
        /// <summary>
        /// Builds the &apos;/api/session/{sessionID}/inbox/{inboxID}&apos; route.
        /// </summary>
        /// <param name = "sessionId">The &apos;sessionID&apos; route value.</param>
        /// <param name = "inboxId">The &apos;inboxID&apos; route value.</param>
        /// <returns>The escaped route.</returns>
        public static string UpdateInbox(string sessionId, string inboxId)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(sessionId);
            if (sessionId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(sessionId));
            }

            ArgumentException.ThrowIfNullOrWhiteSpace(inboxId);
            if (inboxId is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(inboxId));
            }

            return "/api/session/" + RouteValuePolicy.Escape(sessionId, nameof(sessionId)) + "/inbox/" + RouteValuePolicy.Escape(inboxId, nameof(inboxId));
        }
    }

    /// <summary>
    /// Defines the &apos;Shells&apos; routes.
    /// </summary>
    public static class Shells
    {
        /// <summary>
        /// The &apos;POST /api/shell&apos; route template.
        /// </summary>
        public const string CreateShellTemplate = "/api/shell";
        /// <summary>
        /// Builds the &apos;/api/shell&apos; route.
        /// </summary>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string CreateShell(ShellCreateRequest? request = null)
        {
            var path = "/api/shell";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;GET /api/shell/{id}&apos; route template.
        /// </summary>
        public const string GetTemplate = "/api/shell/{id}";
        /// <summary>
        /// Builds the &apos;/api/shell/{id}&apos; route.
        /// </summary>
        /// <param name = "id">The &apos;id&apos; route value.</param>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string Get(string id, ShellRequest? request = null)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(id);
            if (id is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(id));
            }

            var path = "/api/shell/" + RouteValuePolicy.Escape(id, nameof(id));
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;GET /api/shell/{id}/output&apos; route template.
        /// </summary>
        public const string GetOutputTemplate = "/api/shell/{id}/output";
        /// <summary>
        /// Builds the &apos;/api/shell/{id}/output&apos; route.
        /// </summary>
        /// <param name = "id">The &apos;id&apos; route value.</param>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string GetOutput(string id, ShellOutputRequest? request = null)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(id);
            if (id is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(id));
            }

            var path = "/api/shell/" + RouteValuePolicy.Escape(id, nameof(id)) + "/output";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            query.AddText("cursor", request.Cursor);
            query.AddText("limit", request.Limit);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;GET /api/shell&apos; route template.
        /// </summary>
        public const string ListShellsTemplate = "/api/shell";
        /// <summary>
        /// Builds the &apos;/api/shell&apos; route.
        /// </summary>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string ListShells(ShellListRequest? request = null)
        {
            var path = "/api/shell";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;DELETE /api/shell/{id}&apos; route template.
        /// </summary>
        public const string RemoveTemplate = "/api/shell/{id}";
        /// <summary>
        /// Builds the &apos;/api/shell/{id}&apos; route.
        /// </summary>
        /// <param name = "id">The &apos;id&apos; route value.</param>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string Remove(string id, ShellRemoveRequest? request = null)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(id);
            if (id is "." or "..")
            {
                throw new ArgumentException("Route values must not be dot segments.", nameof(id));
            }

            var path = "/api/shell/" + RouteValuePolicy.Escape(id, nameof(id));
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }
    }

    /// <summary>
    /// Defines the &apos;Skills&apos; routes.
    /// </summary>
    public static class Skills
    {
        /// <summary>
        /// The &apos;GET /api/skill&apos; route template.
        /// </summary>
        public const string ListSkillsTemplate = "/api/skill";
        /// <summary>
        /// Builds the &apos;/api/skill&apos; route.
        /// </summary>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string ListSkills(SkillListRequest? request = null)
        {
            var path = "/api/skill";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }
    }

    /// <summary>
    /// Defines the &apos;Vcs&apos; routes.
    /// </summary>
    public static class Vcs
    {
        /// <summary>
        /// The &apos;GET /api/vcs/base&apos; route template.
        /// </summary>
        public const string GetBaseTemplate = "/api/vcs/base";
        /// <summary>
        /// Builds the &apos;/api/vcs/base&apos; route.
        /// </summary>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string GetBase(VcsBaseRequest? request = null)
        {
            var path = "/api/vcs/base";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;GET /api/vcs/diff&apos; route template.
        /// </summary>
        public const string GetDiffTemplate = "/api/vcs/diff";
        /// <summary>
        /// Builds the &apos;/api/vcs/diff&apos; route.
        /// </summary>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string GetDiff(VcsDiffRequest request)
        {
            ArgumentNullException.ThrowIfNull(request);
            var path = "/api/vcs/diff";
            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            query.AddText("mode", ToWireValue(request.Mode));
            query.AddText("base", request.Base);
            query.AddText("context", request.Context);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;GET /api/vcs/status&apos; route template.
        /// </summary>
        public const string GetStatusTemplate = "/api/vcs/status";
        /// <summary>
        /// Builds the &apos;/api/vcs/status&apos; route.
        /// </summary>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string GetStatus(VcsStatusRequest? request = null)
        {
            var path = "/api/vcs/status";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;GET /api/vcs&apos; route template.
        /// </summary>
        public const string GetVcsTemplate = "/api/vcs";
        /// <summary>
        /// Builds the &apos;/api/vcs&apos; route.
        /// </summary>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string GetVcs(VcsRequest? request = null)
        {
            var path = "/api/vcs";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;GET /api/vcs/branch&apos; route template.
        /// </summary>
        public const string ListBranchesTemplate = "/api/vcs/branch";
        /// <summary>
        /// Builds the &apos;/api/vcs/branch&apos; route.
        /// </summary>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string ListBranches(VcsBranchListRequest? request = null)
        {
            var path = "/api/vcs/branch";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            query.AddText("search", request.Search);
            query.AddText("limit", request.Limit);
            return path + query.Value;
        }
    }

    /// <summary>
    /// Defines the &apos;Websearch&apos; routes.
    /// </summary>
    public static class Websearch
    {
        /// <summary>
        /// The &apos;GET /api/websearch/provider&apos; route template.
        /// </summary>
        public const string GetProvidersTemplate = "/api/websearch/provider";
        /// <summary>
        /// Builds the &apos;/api/websearch/provider&apos; route.
        /// </summary>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string GetProviders(WebsearchProvidersRequest? request = null)
        {
            var path = "/api/websearch/provider";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;POST /api/websearch&apos; route template.
        /// </summary>
        public const string QueryTemplate = "/api/websearch";
        /// <summary>
        /// Builds the &apos;/api/websearch&apos; route.
        /// </summary>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string Query(WebsearchQueryRequest? request = null)
        {
            var path = "/api/websearch";
            if (request is null)
            {
                return path;
            }

            var query = new QueryStringBuilder();
            query.AddLocation("location", request.Location);
            return path + query.Value;
        }
    }

    /// <summary>
    /// Defines the &apos;Worktrees&apos; routes.
    /// </summary>
    public static class Worktrees
    {
        /// <summary>
        /// The &apos;POST /api/worktree&apos; route.
        /// </summary>
        public const string CreateWorktree = "/api/worktree";
        /// <summary>
        /// The &apos;GET /api/worktree&apos; route template.
        /// </summary>
        public const string ListWorktreesTemplate = "/api/worktree";
        /// <summary>
        /// Builds the &apos;/api/worktree&apos; route.
        /// </summary>
        /// <param name = "request">The request shaping the query.</param>
        /// <returns>The escaped route.</returns>
        public static string ListWorktrees(WorktreeListRequest request)
        {
            ArgumentNullException.ThrowIfNull(request);
            var path = "/api/worktree";
            var query = new QueryStringBuilder();
            query.AddText("projectID", request.ProjectId);
            return path + query.Value;
        }

        /// <summary>
        /// The &apos;POST /api/worktree/refresh&apos; route.
        /// </summary>
        public const string RefreshWorktrees = "/api/worktree/refresh";
        /// <summary>
        /// The &apos;DELETE /api/worktree&apos; route.
        /// </summary>
        public const string RemoveWorktree = "/api/worktree";
    }

    /// <summary>
    /// Spells one &apos;ExperimentalSessionStatsRequestTools&apos; member the way the wire query expects it.
    /// </summary>
    private static string? ToWireValue(ExperimentalSessionStatsRequestTools? value) => value switch
    {
        ExperimentalSessionStatsRequestTools.None => "none",
        ExperimentalSessionStatsRequestTools.Summary => "summary",
        ExperimentalSessionStatsRequestTools.Detail => "detail",
        null => null,
        _ => throw new ArgumentOutOfRangeException(nameof(value), value, "Unknown ExperimentalSessionStatsRequestTools value.")
    };
    /// <summary>
    /// Spells one &apos;FsFindRequestType&apos; member the way the wire query expects it.
    /// </summary>
    private static string? ToWireValue(FsFindRequestType? value) => value switch
    {
        FsFindRequestType.File => "file",
        FsFindRequestType.Directory => "directory",
        null => null,
        _ => throw new ArgumentOutOfRangeException(nameof(value), value, "Unknown FsFindRequestType value.")
    };
    /// <summary>
    /// Spells one &apos;SessionMessageListRequestType&apos; member the way the wire query expects it.
    /// </summary>
    private static string? ToWireValue(SessionMessageListRequestType? value) => value switch
    {
        SessionMessageListRequestType.AgentSwitched => "agent-switched",
        SessionMessageListRequestType.ModelSwitched => "model-switched",
        SessionMessageListRequestType.LocationSwitched => "location-switched",
        SessionMessageListRequestType.User => "user",
        SessionMessageListRequestType.Synthetic => "synthetic",
        SessionMessageListRequestType.System => "system",
        SessionMessageListRequestType.Skill => "skill",
        SessionMessageListRequestType.Shell => "shell",
        SessionMessageListRequestType.Assistant => "assistant",
        SessionMessageListRequestType.Compaction => "compaction",
        null => null,
        _ => throw new ArgumentOutOfRangeException(nameof(value), value, "Unknown SessionMessageListRequestType value.")
    };
    /// <summary>
    /// Spells one &apos;VcsMode&apos; member the way the wire query expects it.
    /// </summary>
    private static string? ToWireValue(VcsMode? value) => value switch
    {
        VcsMode.Working => "working",
        VcsMode.Branch => "branch",
        VcsMode.Committed => "committed",
        null => null,
        _ => throw new ArgumentOutOfRangeException(nameof(value), value, "Unknown VcsMode value.")
    };
}
