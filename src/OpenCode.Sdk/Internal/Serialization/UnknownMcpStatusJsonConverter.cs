...
internal sealed class UnknownMcpStatusJsonConverter : JsonConverter<UnknownMcpStatus>
{
    public override UnknownMcpStatus Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        {
            throw new JsonException("The McpStatus payload must be a JSON object.");
        }

        if (!payload.TryGetProperty("status", out var markerElement))
        {
            throw new JsonException("The McpStatus payload must contain 'status'.");
        }
    ...
    public override void Write(Utf8JsonWriter writer, UnknownMcpStatus value, JsonSerializerOptions options)
    {
