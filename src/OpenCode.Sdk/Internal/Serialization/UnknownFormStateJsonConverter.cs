...
internal sealed class UnknownFormStateJsonConverter : JsonConverter<UnknownFormState>
{
    public override UnknownFormState Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        {
            throw new JsonException("The FormState payload must be a JSON object.");
        }

        if (!payload.TryGetProperty("status", out var markerElement))
        {
            throw new JsonException("The FormState payload must contain 'status'.");
        }
    ...
    public override void Write(Utf8JsonWriter writer, UnknownFormState value, JsonSerializerOptions options)
    {
