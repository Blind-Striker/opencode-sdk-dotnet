...
internal sealed class UnknownPluginSourceJsonConverter : JsonConverter<UnknownPluginSource>
{
    public override UnknownPluginSource Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        {
            throw new JsonException("The PluginSource payload must be a JSON object.");
        }

        if (!payload.TryGetProperty("type", out var markerElement))
        {
            throw new JsonException("The PluginSource payload must contain 'type'.");
        }
    ...
    public override void Write(Utf8JsonWriter writer, UnknownPluginSource value, JsonSerializerOptions options)
    {
