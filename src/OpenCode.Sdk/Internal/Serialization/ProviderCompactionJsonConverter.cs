...
internal sealed class ProviderCompactionJsonConverter : JsonConverter<IProviderCompaction>
{
    ...
    public override void Write(Utf8JsonWriter writer, IProviderCompaction value, JsonSerializerOptions options)
    {
        ArgumentNullException.ThrowIfNull(writer);
        ArgumentNullException.ThrowIfNull(value);
        ArgumentNullException.ThrowIfNull(options);
        if (value is UnknownProviderCompaction unknown)
        {
            unknown.Payload.WriteTo(writer);
            return;
        }

        var typeInfo = OpenCodeJsonContext.Default.GetTypeInfo(value.GetType()) ?? throw new JsonException("The generated context has no metadata for ProviderCompaction.");
