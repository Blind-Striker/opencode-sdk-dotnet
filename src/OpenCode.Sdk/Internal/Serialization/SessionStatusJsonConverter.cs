...
internal sealed class SessionStatusJsonConverter : JsonConverter<ISessionStatus>
{
    ...
    public override ISessionStatus Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        {
            throw new JsonException("The SessionStatus payload must be a JSON object.");
        }

        if (!payload.TryGetProperty("type", out var markerElement))
        {
            throw new JsonException("The SessionStatus payload must contain 'type'.");
        }
        ...
        if (TypesByTag.TryGetValue(marker, out var targetType))
        {
            ...
            return JsonSerializer.Deserialize(payload, typeInfo) as ISessionStatus ?? throw new JsonException("The SessionStatus payload deserialized to null.");
        }
