...
/// <summary>
/// What one status probe learned, in the pinned client's own return shape (<c>probeResult</c>):
...
/// <param name="Version">The version the status answer reported, when there was one.</param>
...
internal sealed record ServiceProbeResult(ServiceState? State, string? Version, bool TimedOut)
{
    ...
    /// an authenticated 404 on the status route as the registered daemon, present and ready, but
