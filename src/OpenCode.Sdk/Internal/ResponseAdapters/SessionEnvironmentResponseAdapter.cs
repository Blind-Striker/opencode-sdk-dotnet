...
/// <summary>
/// Adapts the &apos;PUT /api/session/{sessionID}/environment&apos; responses onto &apos;SessionEnvironmentResponse&apos;.
/// </summary>
internal sealed class SessionEnvironmentResponseAdapter : ResponseAdapter<SessionEnvironmentResponse>
{
    ...
    private SessionEnvironmentResponseAdapter()
    {
    }
    ...
    /// </summary>
    public static SessionEnvironmentResponseAdapter Instance { get; } = new SessionEnvironmentResponseAdapter();
    ...
    /// </summary>
    public override SessionEnvironmentResponse AdaptSuccess(int status, ReadOnlySpan<byte> utf8Body) => new()
    {
        Status = status
    };
    /// <summary>
    ...
    /// </summary>
    public override SessionEnvironmentResponse Adapt(int status, string rawBody)
    {
        ArgumentNullException.ThrowIfNull(rawBody);
        return status switch
        {
            204 => new SessionEnvironmentResponse
            {
                Status = status
            },
            >= 200 and < 300 => throw StatusVerdictFailures.UndeclaredSuccess(status),
            400 => new SessionEnvironmentResponse(status, ReadTolerantError(rawBody, Status400Tags), rawBody),
            401 => new SessionEnvironmentResponse(status, ReadTolerantError(rawBody, Status401Tags), rawBody),
            404 => new SessionEnvironmentResponse(status, ReadTolerantError(rawBody, Status404Tags), rawBody),
            _ => new SessionEnvironmentResponse(status, ReadTolerantError(rawBody, null), rawBody)
        };
    }
}
