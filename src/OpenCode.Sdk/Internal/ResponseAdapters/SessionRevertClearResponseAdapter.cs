...
/// <summary>
/// Adapts the &apos;DELETE /api/session/{sessionID}/revert&apos; responses onto &apos;SessionRevertClearResponse&apos;.
/// </summary>
internal sealed class SessionRevertClearResponseAdapter : ResponseAdapter<SessionRevertClearResponse>
{
    ...
    /// </summary>
    public static SessionRevertClearResponseAdapter Instance { get; } = new SessionRevertClearResponseAdapter();
    ...
    /// </summary>
    public override SessionRevertClearResponse AdaptSuccess(int status, ReadOnlySpan<byte> utf8Body) => new()
    {
        Status = status
    };
    /// <summary>
    ...
    /// </summary>
    public override SessionRevertClearResponse Adapt(int status, string rawBody)
    {
        ArgumentNullException.ThrowIfNull(rawBody);
        return status switch
        {
            204 => new SessionRevertClearResponse
            {
                Status = status
            },
            >= 200 and < 300 => throw StatusVerdictFailures.UndeclaredSuccess(status),
            400 => new SessionRevertClearResponse(status, ReadTolerantError(rawBody, Status400Tags), rawBody),
            401 => new SessionRevertClearResponse(status, ReadTolerantError(rawBody, Status401Tags), rawBody),
            404 => new SessionRevertClearResponse(status, ReadTolerantError(rawBody, Status404Tags), rawBody),
            409 => new SessionRevertClearResponse(status, ReadTolerantError(rawBody, Status409Tags), rawBody),
            500 => new SessionRevertClearResponse(status, ReadTolerantError(rawBody, Status500Tags), rawBody),
            _ => new SessionRevertClearResponse(status, ReadTolerantError(rawBody, null), rawBody)
        };
    }
}
