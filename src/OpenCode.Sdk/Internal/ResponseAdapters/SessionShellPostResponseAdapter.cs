...
/// <summary>
/// Adapts the &apos;POST /api/session/{sessionID}/shell&apos; responses onto &apos;SessionShellPostResponse&apos;.
/// </summary>
internal sealed class SessionShellPostResponseAdapter : ResponseAdapter<SessionShellPostResponse>
{
    ...
    private SessionShellPostResponseAdapter()
    {
    }
    ...
    /// </summary>
    public static SessionShellPostResponseAdapter Instance { get; } = new SessionShellPostResponseAdapter();
    ...
    /// </summary>
    public override SessionShellPostResponse AdaptSuccess(int status, ReadOnlySpan<byte> utf8Body) => new()
    {
        Status = status
    };
    /// <summary>
    ...
    /// </summary>
    public override SessionShellPostResponse Adapt(int status, string rawBody)
    {
        ArgumentNullException.ThrowIfNull(rawBody);
        return status switch
        {
            204 => new SessionShellPostResponse
            {
                Status = status
            },
            >= 200 and < 300 => throw StatusVerdictFailures.UndeclaredSuccess(status),
            400 => new SessionShellPostResponse(status, ReadTolerantError(rawBody, Status400Tags), rawBody),
            401 => new SessionShellPostResponse(status, ReadTolerantError(rawBody, Status401Tags), rawBody),
            404 => new SessionShellPostResponse(status, ReadTolerantError(rawBody, Status404Tags), rawBody),
            _ => new SessionShellPostResponse(status, ReadTolerantError(rawBody, null), rawBody)
        };
    }
}
