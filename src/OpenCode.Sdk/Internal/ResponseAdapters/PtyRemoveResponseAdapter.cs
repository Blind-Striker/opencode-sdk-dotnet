...
/// <summary>
/// Adapts the &apos;DELETE /api/pty/{ptyID}&apos; responses onto &apos;PtyRemoveResponse&apos;.
/// </summary>
internal sealed class PtyRemoveResponseAdapter : ResponseAdapter<PtyRemoveResponse>
{
    ...
    private PtyRemoveResponseAdapter()
    {
    }
    ...
    /// </summary>
    public static PtyRemoveResponseAdapter Instance { get; } = new PtyRemoveResponseAdapter();
    ...
    /// </summary>
    public override PtyRemoveResponse AdaptSuccess(int status, ReadOnlySpan<byte> utf8Body) => new()
    {
        Status = status
    };
    /// <summary>
    ...
    /// </summary>
    public override PtyRemoveResponse Adapt(int status, string rawBody)
    {
        ArgumentNullException.ThrowIfNull(rawBody);
        return status switch
        {
            204 => new PtyRemoveResponse
            {
                Status = status
            },
            >= 200 and < 300 => throw StatusVerdictFailures.UndeclaredSuccess(status),
            400 => new PtyRemoveResponse(status, ReadTolerantError(rawBody, Status400Tags), rawBody),
            401 => new PtyRemoveResponse(status, ReadTolerantError(rawBody, Status401Tags), rawBody),
            404 => new PtyRemoveResponse(status, ReadTolerantError(rawBody, Status404Tags), rawBody),
            _ => new PtyRemoveResponse(status, ReadTolerantError(rawBody, null), rawBody)
        };
    }
}
