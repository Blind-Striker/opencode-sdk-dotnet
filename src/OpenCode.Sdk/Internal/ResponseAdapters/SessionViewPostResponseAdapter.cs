...
/// <summary>
/// Adapts the &apos;POST /api/session/{sessionID}/view&apos; responses onto &apos;SessionViewPostResponse&apos;.
/// </summary>
internal sealed class SessionViewPostResponseAdapter : ResponseAdapter<SessionViewPostResponse>
{
    ...
    private SessionViewPostResponseAdapter()
    {
    }
    ...
    /// </summary>
    public static SessionViewPostResponseAdapter Instance { get; } = new SessionViewPostResponseAdapter();
    ...
    /// </summary>
    public override SessionViewPostResponse AdaptSuccess(int status, ReadOnlySpan<byte> utf8Body) => new()
    {
        Status = status
    };
    /// <summary>
    ...
    /// </summary>
    public override SessionViewPostResponse Adapt(int status, string rawBody)
    {
        ArgumentNullException.ThrowIfNull(rawBody);
        return status switch
        {
            204 => new SessionViewPostResponse
            {
                Status = status
            },
            >= 200 and < 300 => throw StatusVerdictFailures.UndeclaredSuccess(status),
            400 => new SessionViewPostResponse(status, ReadTolerantError(rawBody, Status400Tags), rawBody),
            401 => new SessionViewPostResponse(status, ReadTolerantError(rawBody, Status401Tags), rawBody),
            404 => new SessionViewPostResponse(status, ReadTolerantError(rawBody, Status404Tags), rawBody),
            _ => new SessionViewPostResponse(status, ReadTolerantError(rawBody, null), rawBody)
        };
    }
}
