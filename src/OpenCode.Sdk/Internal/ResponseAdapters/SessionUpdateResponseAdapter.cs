...
/// <summary>
/// Adapts the &apos;PATCH /api/session/{sessionID}&apos; responses onto &apos;SessionUpdateResponse&apos;.
/// </summary>
internal sealed class SessionUpdateResponseAdapter : ResponseAdapter<SessionUpdateResponse>
{
    ...
    /// </summary>
    public static SessionUpdateResponseAdapter Instance { get; } = new SessionUpdateResponseAdapter();
    ...
    /// </summary>
    public override SessionUpdateResponse AdaptSuccess(int status, ReadOnlySpan<byte> utf8Body) => new()
    {
        Status = status
    };
    /// <summary>
    ...
    /// </summary>
    public override SessionUpdateResponse Adapt(int status, string rawBody)
    {
        ArgumentNullException.ThrowIfNull(rawBody);
        return status switch
        {
            204 => new SessionUpdateResponse
            {
                Status = status
            },
            >= 200 and < 300 => throw StatusVerdictFailures.UndeclaredSuccess(status),
            400 => new SessionUpdateResponse(status, ReadTolerantError(rawBody, Status400Tags), rawBody),
            401 => new SessionUpdateResponse(status, ReadTolerantError(rawBody, Status401Tags), rawBody),
            404 => new SessionUpdateResponse(status, ReadTolerantError(rawBody, Status404Tags), rawBody),
            _ => new SessionUpdateResponse(status, ReadTolerantError(rawBody, null), rawBody)
        };
    }
}
