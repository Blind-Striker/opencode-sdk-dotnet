...
/// <summary>
/// Adapts the &apos;PATCH /api/session/{sessionID}/inbox/{inboxID}&apos; responses onto &apos;SessionInboxUpdateResponse&apos;.
/// </summary>
internal sealed class SessionInboxUpdateResponseAdapter : ResponseAdapter<SessionInboxUpdateResponse>
{
    ...
    private SessionInboxUpdateResponseAdapter()
    {
    }
    ...
    /// </summary>
    public static SessionInboxUpdateResponseAdapter Instance { get; } = new SessionInboxUpdateResponseAdapter();
    ...
    /// </summary>
    public override SessionInboxUpdateResponse AdaptSuccess(int status, ReadOnlySpan<byte> utf8Body) => new()
    {
        Status = status
    };
    /// <summary>
    ...
    /// </summary>
    public override SessionInboxUpdateResponse Adapt(int status, string rawBody)
    {
        ArgumentNullException.ThrowIfNull(rawBody);
        return status switch
        {
            204 => new SessionInboxUpdateResponse
            {
                Status = status
            },
            >= 200 and < 300 => throw StatusVerdictFailures.UndeclaredSuccess(status),
            400 => new SessionInboxUpdateResponse(status, ReadTolerantError(rawBody, Status400Tags), rawBody),
            401 => new SessionInboxUpdateResponse(status, ReadTolerantError(rawBody, Status401Tags), rawBody),
            404 => new SessionInboxUpdateResponse(status, ReadTolerantError(rawBody, Status404Tags), rawBody),
            409 => new SessionInboxUpdateResponse(status, ReadTolerantError(rawBody, Status409Tags), rawBody),
            _ => new SessionInboxUpdateResponse(status, ReadTolerantError(rawBody, null), rawBody)
        };
    }
}
