...
/// <summary>
/// Adapts the &apos;POST /api/session/{sessionID}/shell&apos; responses onto &apos;SessionShellResponse&apos;.
/// </summary>
internal sealed class SessionShellResponseAdapter : ResponseAdapter<SessionShellResponse>
{
    ...
    private SessionShellResponseAdapter()
    {
    }
    ...
    /// </summary>
    public static SessionShellResponseAdapter Instance { get; } = new SessionShellResponseAdapter();
    ...
    /// </summary>
    public override SessionShellResponse AdaptSuccess(int status, ReadOnlySpan<byte> utf8Body) => new()
    {
        Status = status
    };
    /// <summary>
    ...
    /// </summary>
    public override SessionShellResponse Adapt(int status, string rawBody)
    {
        ArgumentNullException.ThrowIfNull(rawBody);
        return status switch
        {
            204 => new SessionShellResponse
            {
                Status = status
            },
            >= 200 and < 300 => throw StatusVerdictFailures.UndeclaredSuccess(status),
            400 => new SessionShellResponse(status, ReadTolerantError(rawBody, Status400Tags), rawBody),
            401 => new SessionShellResponse(status, ReadTolerantError(rawBody, Status401Tags), rawBody),
            404 => new SessionShellResponse(status, ReadTolerantError(rawBody, Status404Tags), rawBody),
            _ => new SessionShellResponse(status, ReadTolerantError(rawBody, null), rawBody)
        };
    }
}
