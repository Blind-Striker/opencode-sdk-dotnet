...
/// <summary>
/// Adapts the &apos;POST /api/session/{sessionID}/form/{formID}/reply&apos; responses onto &apos;SessionFormReplyResponse&apos;.
/// </summary>
internal sealed class SessionFormReplyResponseAdapter : ResponseAdapter<SessionFormReplyResponse>
{
    ...
    /// </summary>
    public static SessionFormReplyResponseAdapter Instance { get; } = new SessionFormReplyResponseAdapter();
    ...
    /// </summary>
    public override SessionFormReplyResponse AdaptSuccess(int status, ReadOnlySpan<byte> utf8Body) => new()
    {
        Status = status
    };
    /// <summary>
    ...
    /// </summary>
    public override SessionFormReplyResponse Adapt(int status, string rawBody)
    {
        ArgumentNullException.ThrowIfNull(rawBody);
        return status switch
        {
            204 => new SessionFormReplyResponse
            {
                Status = status
            },
            >= 200 and < 300 => throw StatusVerdictFailures.UndeclaredSuccess(status),
            400 => new SessionFormReplyResponse(status, ReadTolerantError(rawBody, Status400Tags), rawBody),
            401 => new SessionFormReplyResponse(status, ReadTolerantError(rawBody, Status401Tags), rawBody),
            404 => new SessionFormReplyResponse(status, ReadTolerantError(rawBody, Status404Tags), rawBody),
            409 => new SessionFormReplyResponse(status, ReadTolerantError(rawBody, Status409Tags), rawBody),
            _ => new SessionFormReplyResponse(status, ReadTolerantError(rawBody, null), rawBody)
        };
    }
}
