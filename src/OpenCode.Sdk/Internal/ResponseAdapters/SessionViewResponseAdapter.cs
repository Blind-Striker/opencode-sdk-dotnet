...
/// <summary>
/// Adapts the &apos;POST /api/session/{sessionID}/view&apos; responses onto &apos;SessionViewResponse&apos;.
/// </summary>
internal sealed class SessionViewResponseAdapter : ResponseAdapter<SessionViewResponse>
{
    ...
    private SessionViewResponseAdapter()
    {
    }
    ...
    /// </summary>
    public static SessionViewResponseAdapter Instance { get; } = new SessionViewResponseAdapter();
    ...
    /// </summary>
    public override SessionViewResponse AdaptSuccess(int status, ReadOnlySpan<byte> utf8Body) => new()
    {
        Status = status
    };
    /// <summary>
    ...
    /// </summary>
    public override SessionViewResponse Adapt(int status, string rawBody)
    {
        ArgumentNullException.ThrowIfNull(rawBody);
        return status switch
        {
            204 => new SessionViewResponse
            {
                Status = status
            },
            >= 200 and < 300 => throw StatusVerdictFailures.UndeclaredSuccess(status),
            400 => new SessionViewResponse(status, ReadTolerantError(rawBody, Status400Tags), rawBody),
            401 => new SessionViewResponse(status, ReadTolerantError(rawBody, Status401Tags), rawBody),
            404 => new SessionViewResponse(status, ReadTolerantError(rawBody, Status404Tags), rawBody),
            _ => new SessionViewResponse(status, ReadTolerantError(rawBody, null), rawBody)
        };
    }
}
