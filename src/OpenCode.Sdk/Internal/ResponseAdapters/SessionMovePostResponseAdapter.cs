...
/// <summary>
/// Adapts the &apos;POST /api/session/{sessionID}/move&apos; responses onto &apos;SessionMovePostResponse&apos;.
/// </summary>
internal sealed class SessionMovePostResponseAdapter : ResponseAdapter<SessionMovePostResponse>
{
    ...
    private SessionMovePostResponseAdapter()
    {
    }
    ...
    /// </summary>
    public static SessionMovePostResponseAdapter Instance { get; } = new SessionMovePostResponseAdapter();
    ...
    /// </summary>
    public override SessionMovePostResponse AdaptSuccess(int status, ReadOnlySpan<byte> utf8Body) => new()
    {
        Status = status
    };
    /// <summary>
    ...
    /// </summary>
    public override SessionMovePostResponse Adapt(int status, string rawBody)
    {
        ArgumentNullException.ThrowIfNull(rawBody);
        return status switch
        {
            204 => new SessionMovePostResponse
            {
                Status = status
            },
            >= 200 and < 300 => throw StatusVerdictFailures.UndeclaredSuccess(status),
            400 => new SessionMovePostResponse(status, ReadTolerantError(rawBody, Status400Tags), rawBody),
            401 => new SessionMovePostResponse(status, ReadTolerantError(rawBody, Status401Tags), rawBody),
            404 => new SessionMovePostResponse(status, ReadTolerantError(rawBody, Status404Tags), rawBody),
            _ => new SessionMovePostResponse(status, ReadTolerantError(rawBody, null), rawBody)
        };
    }
}
