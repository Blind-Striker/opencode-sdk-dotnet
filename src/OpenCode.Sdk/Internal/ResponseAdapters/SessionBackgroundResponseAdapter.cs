...
/// <summary>
/// Adapts the &apos;POST /api/session/{sessionID}/background&apos; responses onto &apos;SessionBackgroundResponse&apos;.
/// </summary>
internal sealed class SessionBackgroundResponseAdapter : ResponseAdapter<SessionBackgroundResponse>
{
    ...
    private SessionBackgroundResponseAdapter()
    {
    }
    ...
    /// </summary>
    public static SessionBackgroundResponseAdapter Instance { get; } = new SessionBackgroundResponseAdapter();
    ...
    /// </summary>
    public override SessionBackgroundResponse AdaptSuccess(int status, ReadOnlySpan<byte> utf8Body) => new()
    {
        Status = status
    };
    /// <summary>
    ...
    /// </summary>
    public override SessionBackgroundResponse Adapt(int status, string rawBody)
    {
        ArgumentNullException.ThrowIfNull(rawBody);
        return status switch
        {
            204 => new SessionBackgroundResponse
            {
                Status = status
            },
            >= 200 and < 300 => throw StatusVerdictFailures.UndeclaredSuccess(status),
            400 => new SessionBackgroundResponse(status, ReadTolerantError(rawBody, Status400Tags), rawBody),
            401 => new SessionBackgroundResponse(status, ReadTolerantError(rawBody, Status401Tags), rawBody),
            404 => new SessionBackgroundResponse(status, ReadTolerantError(rawBody, Status404Tags), rawBody),
            _ => new SessionBackgroundResponse(status, ReadTolerantError(rawBody, null), rawBody)
        };
    }
}
