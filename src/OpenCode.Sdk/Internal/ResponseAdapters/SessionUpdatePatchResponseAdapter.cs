...
/// <summary>
/// Adapts the &apos;PATCH /api/session/{sessionID}&apos; responses onto &apos;SessionUpdatePatchResponse&apos;.
/// </summary>
internal sealed class SessionUpdatePatchResponseAdapter : ResponseAdapter<SessionUpdatePatchResponse>
{
    ...
    private SessionUpdatePatchResponseAdapter()
    {
    }
    ...
    /// </summary>
    public static SessionUpdatePatchResponseAdapter Instance { get; } = new SessionUpdatePatchResponseAdapter();
    ...
    /// </summary>
    public override SessionUpdatePatchResponse AdaptSuccess(int status, ReadOnlySpan<byte> utf8Body) => new()
    {
        Status = status
    };
    /// <summary>
    ...
    /// </summary>
    public override SessionUpdatePatchResponse Adapt(int status, string rawBody)
    {
        ArgumentNullException.ThrowIfNull(rawBody);
        return status switch
        {
            204 => new SessionUpdatePatchResponse
            {
                Status = status
            },
            >= 200 and < 300 => throw StatusVerdictFailures.UndeclaredSuccess(status),
            400 => new SessionUpdatePatchResponse(status, ReadTolerantError(rawBody, Status400Tags), rawBody),
            401 => new SessionUpdatePatchResponse(status, ReadTolerantError(rawBody, Status401Tags), rawBody),
            404 => new SessionUpdatePatchResponse(status, ReadTolerantError(rawBody, Status404Tags), rawBody),
            _ => new SessionUpdatePatchResponse(status, ReadTolerantError(rawBody, null), rawBody)
        };
    }
}
