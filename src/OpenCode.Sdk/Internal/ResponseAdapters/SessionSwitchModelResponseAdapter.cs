...
/// <summary>
/// Adapts the &apos;POST /api/session/{sessionID}/model&apos; responses onto &apos;SessionSwitchModelResponse&apos;.
/// </summary>
internal sealed class SessionSwitchModelResponseAdapter : ResponseAdapter<SessionSwitchModelResponse>
{
    ...
    private SessionSwitchModelResponseAdapter()
    {
    }
    ...
    /// </summary>
    public static SessionSwitchModelResponseAdapter Instance { get; } = new SessionSwitchModelResponseAdapter();
    ...
    /// </summary>
    public override SessionSwitchModelResponse AdaptSuccess(int status, ReadOnlySpan<byte> utf8Body) => new()
    {
        Status = status
    };
    /// <summary>
    ...
    /// </summary>
    public override SessionSwitchModelResponse Adapt(int status, string rawBody)
    {
        ArgumentNullException.ThrowIfNull(rawBody);
        return status switch
        {
            204 => new SessionSwitchModelResponse
            {
                Status = status
            },
            >= 200 and < 300 => throw StatusVerdictFailures.UndeclaredSuccess(status),
            400 => new SessionSwitchModelResponse(status, ReadTolerantError(rawBody, Status400Tags), rawBody),
            401 => new SessionSwitchModelResponse(status, ReadTolerantError(rawBody, Status401Tags), rawBody),
            404 => new SessionSwitchModelResponse(status, ReadTolerantError(rawBody, Status404Tags), rawBody),
            _ => new SessionSwitchModelResponse(status, ReadTolerantError(rawBody, null), rawBody)
        };
    }
}
