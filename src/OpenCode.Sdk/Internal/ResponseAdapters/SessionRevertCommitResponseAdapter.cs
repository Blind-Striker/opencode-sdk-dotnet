...
/// <summary>
/// Adapts the &apos;POST /api/session/{sessionID}/revert/commit&apos; responses onto &apos;SessionRevertCommitResponse&apos;.
/// </summary>
internal sealed class SessionRevertCommitResponseAdapter : ResponseAdapter<SessionRevertCommitResponse>
{
    ...
    private SessionRevertCommitResponseAdapter()
    {
    }
    ...
    /// </summary>
    public static SessionRevertCommitResponseAdapter Instance { get; } = new SessionRevertCommitResponseAdapter();
    ...
    /// </summary>
    public override SessionRevertCommitResponse AdaptSuccess(int status, ReadOnlySpan<byte> utf8Body) => new()
    {
        Status = status
    };
    /// <summary>
    ...
    /// </summary>
    public override SessionRevertCommitResponse Adapt(int status, string rawBody)
    {
        ArgumentNullException.ThrowIfNull(rawBody);
        return status switch
        {
            204 => new SessionRevertCommitResponse
            {
                Status = status
            },
            >= 200 and < 300 => throw StatusVerdictFailures.UndeclaredSuccess(status),
            400 => new SessionRevertCommitResponse(status, ReadTolerantError(rawBody, Status400Tags), rawBody),
            401 => new SessionRevertCommitResponse(status, ReadTolerantError(rawBody, Status401Tags), rawBody),
            404 => new SessionRevertCommitResponse(status, ReadTolerantError(rawBody, Status404Tags), rawBody),
            409 => new SessionRevertCommitResponse(status, ReadTolerantError(rawBody, Status409Tags), rawBody),
            _ => new SessionRevertCommitResponse(status, ReadTolerantError(rawBody, null), rawBody)
        };
    }
}
