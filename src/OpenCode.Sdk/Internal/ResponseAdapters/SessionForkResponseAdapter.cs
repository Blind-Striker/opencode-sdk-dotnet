...
/// <summary>
/// Adapts the &apos;POST /api/session/{sessionID}/fork&apos; responses onto &apos;SessionForkResponse&apos;.
/// </summary>
internal sealed class SessionForkResponseAdapter : ResponseAdapter<SessionForkResponse>
{
    ...
    /// </summary>
    public static SessionForkResponseAdapter Instance { get; } = new SessionForkResponseAdapter();
    ...
    /// </summary>
    public override SessionForkResponse AdaptSuccess(int status, ReadOnlySpan<byte> utf8Body) => new()
    {
        Status = status,
        Fork = ReadBarePayload(utf8Body, OpenCodeJsonContext.Default.SessionForkResponseEnvelope).Data
    };
    /// <summary>
    /// Maps one buffered response onto the typed envelope.
    /// </summary>
    public override SessionForkResponse Adapt(int status, string rawBody)
    {
        ArgumentNullException.ThrowIfNull(rawBody);
        return status switch
        {
            200 => new SessionForkResponse
            {
                Status = status,
                Fork = ReadBarePayload(rawBody, OpenCodeJsonContext.Default.SessionForkResponseEnvelope).Data
            },
            >= 200 and < 300 => throw StatusVerdictFailures.UndeclaredSuccess(status),
            400 => new SessionForkResponse(status, ReadTolerantError(rawBody, Status400Tags), rawBody),
            401 => new SessionForkResponse(status, ReadTolerantError(rawBody, Status401Tags), rawBody),
            404 => new SessionForkResponse(status, ReadTolerantError(rawBody, Status404Tags), rawBody),
            _ => new SessionForkResponse(status, ReadTolerantError(rawBody, null), rawBody)
        };
    }
}
