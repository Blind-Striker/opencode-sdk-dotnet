...
/// <summary>
/// Adapts the &apos;DELETE /api/session/{sessionID}/inbox/{inboxID}&apos; responses onto &apos;SessionInboxCancelResponse&apos;.
/// </summary>
internal sealed class SessionInboxCancelResponseAdapter : ResponseAdapter<SessionInboxCancelResponse>
{
    ...
    private SessionInboxCancelResponseAdapter()
    {
    }
    ...
    /// </summary>
    public static SessionInboxCancelResponseAdapter Instance { get; } = new SessionInboxCancelResponseAdapter();
    ...
    /// </summary>
    public override SessionInboxCancelResponse AdaptSuccess(int status, ReadOnlySpan<byte> utf8Body) => new()
    {
        Status = status
    };
    /// <summary>
    ...
    /// </summary>
    public override SessionInboxCancelResponse Adapt(int status, string rawBody)
    {
        ArgumentNullException.ThrowIfNull(rawBody);
        return status switch
        {
            204 => new SessionInboxCancelResponse
            {
                Status = status
            },
            >= 200 and < 300 => throw StatusVerdictFailures.UndeclaredSuccess(status),
            400 => new SessionInboxCancelResponse(status, ReadTolerantError(rawBody, Status400Tags), rawBody),
            401 => new SessionInboxCancelResponse(status, ReadTolerantError(rawBody, Status401Tags), rawBody),
            404 => new SessionInboxCancelResponse(status, ReadTolerantError(rawBody, Status404Tags), rawBody),
            _ => new SessionInboxCancelResponse(status, ReadTolerantError(rawBody, null), rawBody)
        };
    }
}
