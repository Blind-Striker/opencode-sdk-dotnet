...
/// <summary>
/// Adapts the &apos;DELETE /api/mcp/{server}&apos; responses onto &apos;McpRemoveResponse&apos;.
/// </summary>
internal sealed class McpRemoveResponseAdapter : ResponseAdapter<McpRemoveResponse>
{
    ...
    private McpRemoveResponseAdapter()
    {
    }
    ...
    /// </summary>
    public static McpRemoveResponseAdapter Instance { get; } = new McpRemoveResponseAdapter();
    ...
    /// </summary>
    public override McpRemoveResponse AdaptSuccess(int status, ReadOnlySpan<byte> utf8Body) => new()
    {
        Status = status
    };
    /// <summary>
    ...
    /// </summary>
    public override McpRemoveResponse Adapt(int status, string rawBody)
    {
        ArgumentNullException.ThrowIfNull(rawBody);
        return status switch
        {
            204 => new McpRemoveResponse
            {
                Status = status
            },
            >= 200 and < 300 => throw StatusVerdictFailures.UndeclaredSuccess(status),
            400 => new McpRemoveResponse(status, ReadTolerantError(rawBody, Status400Tags), rawBody),
            401 => new McpRemoveResponse(status, ReadTolerantError(rawBody, Status401Tags), rawBody),
            404 => new McpRemoveResponse(status, ReadTolerantError(rawBody, Status404Tags), rawBody),
            _ => new McpRemoveResponse(status, ReadTolerantError(rawBody, null), rawBody)
        };
    }
}
