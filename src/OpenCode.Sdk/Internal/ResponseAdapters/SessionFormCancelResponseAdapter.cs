...
/// <summary>
/// Adapts the &apos;DELETE /api/session/{sessionID}/form/{formID}&apos; responses onto &apos;SessionFormCancelResponse&apos;.
/// </summary>
internal sealed class SessionFormCancelResponseAdapter : ResponseAdapter<SessionFormCancelResponse>
{
    ...
    private SessionFormCancelResponseAdapter()
    {
    }
    ...
    /// </summary>
    public static SessionFormCancelResponseAdapter Instance { get; } = new SessionFormCancelResponseAdapter();
    ...
    /// </summary>
    public override SessionFormCancelResponse AdaptSuccess(int status, ReadOnlySpan<byte> utf8Body) => new()
    {
        Status = status
    };
    /// <summary>
    ...
    /// </summary>
    public override SessionFormCancelResponse Adapt(int status, string rawBody)
    {
        ArgumentNullException.ThrowIfNull(rawBody);
        return status switch
        {
            204 => new SessionFormCancelResponse
            {
                Status = status
            },
            >= 200 and < 300 => throw StatusVerdictFailures.UndeclaredSuccess(status),
            400 => new SessionFormCancelResponse(status, ReadTolerantError(rawBody, Status400Tags), rawBody),
            401 => new SessionFormCancelResponse(status, ReadTolerantError(rawBody, Status401Tags), rawBody),
            404 => new SessionFormCancelResponse(status, ReadTolerantError(rawBody, Status404Tags), rawBody),
            409 => new SessionFormCancelResponse(status, ReadTolerantError(rawBody, Status409Tags), rawBody),
            _ => new SessionFormCancelResponse(status, ReadTolerantError(rawBody, null), rawBody)
        };
    }
}
