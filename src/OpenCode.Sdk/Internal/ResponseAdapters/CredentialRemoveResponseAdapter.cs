...
/// <summary>
/// Adapts the &apos;DELETE /api/credential/{credentialID}&apos; responses onto &apos;CredentialRemoveResponse&apos;.
/// </summary>
internal sealed class CredentialRemoveResponseAdapter : ResponseAdapter<CredentialRemoveResponse>
{
    ...
    private CredentialRemoveResponseAdapter()
    {
    }
    ...
    /// </summary>
    public static CredentialRemoveResponseAdapter Instance { get; } = new CredentialRemoveResponseAdapter();
    ...
    /// </summary>
    public override CredentialRemoveResponse AdaptSuccess(int status, ReadOnlySpan<byte> utf8Body) => new()
    {
        Status = status
    };
    /// <summary>
    ...
    /// </summary>
    public override CredentialRemoveResponse Adapt(int status, string rawBody)
    {
        ArgumentNullException.ThrowIfNull(rawBody);
        return status switch
        {
            204 => new CredentialRemoveResponse
            {
                Status = status
            },
            >= 200 and < 300 => throw StatusVerdictFailures.UndeclaredSuccess(status),
            400 => new CredentialRemoveResponse(status, ReadTolerantError(rawBody, Status400Tags), rawBody),
            401 => new CredentialRemoveResponse(status, ReadTolerantError(rawBody, Status401Tags), rawBody),
            _ => new CredentialRemoveResponse(status, ReadTolerantError(rawBody, null), rawBody)
        };
    }
}
