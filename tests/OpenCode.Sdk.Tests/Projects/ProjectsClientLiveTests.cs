...
public sealed class ProjectsClientLiveTests(SimulatedDriveServerFixture server)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        await Assert.That(current.Current.Directory).IsEqualTo(workspace.Path);
        await Assert.That(current.Current.Canonical).IsEqualTo(workspace.Path);
        ...
        await Assert.That(project.Canonical).IsEqualTo(workspace.Path);
