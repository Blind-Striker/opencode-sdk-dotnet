...
internal static class GeneratedSourceCompiler
{
    ...
    public static async Task<Diagnostic[]> CompileWithSdkCoreAsync(IReadOnlyList<GeneratedSource> sources)
    {
                     ...
                     .OrderBy(static entry => entry.Relative, StringComparer.Ordinal))
        {
            ...
            // The committed generated layer has its own radar (generate --verify); the probe
            // pairs the freshly emitted layer with the hand-written core only.
        ...
        return Compile(sources, coreTrees);
    }

    private static Diagnostic[] Compile(IReadOnlyList<GeneratedSource> sources, IReadOnlyList<SyntaxTree> extraTrees)
    {
